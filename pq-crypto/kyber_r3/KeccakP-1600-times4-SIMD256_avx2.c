...
static const UINT64 rho56[4] = { 0x0007060504030201, 0x080F0E0D0C0B0A09, 0x1017161514131211, 0x181F1E1D1C1B1A19 };
        ...
        #define UNINTLEAVE()                                                                        \
            lanesL01 = UNPACKL(lanes0, lanes1), lanesH01 = UNPACKH(lanes0, lanes1),                 \
            lanesL23 = UNPACKL(lanes2, lanes3), lanesH23 = UNPACKH(lanes2, lanes3),                 \
            lanes0 = PERM128(lanesL01, lanesL23, 0x20), lanes2 = PERM128(lanesL01, lanesL23, 0x31), \
            lanes1 = PERM128(lanesH01, lanesH23, 0x20), lanes3 = PERM128(lanesH01, lanesH23, 0x31)

        #define INTLEAVE()                                                                              \
            lanesL01 = PERM128(lanes0, lanes2, 0x20), lanesH01 = PERM128(lanes1, lanes3, 0x20),         \
            lanesL23 = PERM128(lanes0, lanes2, 0x31), lanesH23 = PERM128(lanes1, lanes3, 0x31),         \
            lanes0 = SHUFFLE64(lanesL01, lanesH01, 0x00), lanes1 = SHUFFLE64(lanesL01, lanesH01, 0x0F), \
            lanes2 = SHUFFLE64(lanesL23, lanesH23, 0x00), lanes3 = SHUFFLE64(lanesL23, lanesH23, 0x0F)
...
void KeccakP1600times4_AddBytes(
    void *states, unsigned int instanceIndex, const unsigned char *data, unsigned int offset, unsigned int length)
{
...
void KeccakP1600times4_AddLanesAll(
    void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    ...
    #define Xor_In(argIndex)             \
        XOReq256(stateAsLanes[argIndex], \
            LOAD4_64(curData3[argIndex], curData2[argIndex], curData1[argIndex], curData0[argIndex]))

    #define Xor_In4(argIndex)                                                                       \
        lanes0 = LOAD256u(curData0[argIndex]), lanes1 = LOAD256u(curData1[argIndex]),               \
        lanes2 = LOAD256u(curData2[argIndex]), lanes3 = LOAD256u(curData3[argIndex]), INTLEAVE(),   \
        XOReq256(stateAsLanes[argIndex + 0], lanes0), XOReq256(stateAsLanes[argIndex + 1], lanes1), \
        XOReq256(stateAsLanes[argIndex + 2], lanes2), XOReq256(stateAsLanes[argIndex + 3], lanes3)
    ...
    void *states, unsigned int instanceIndex, const unsigned char *data, unsigned int offset, unsigned int length)
{
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        if (bytesInLane > sizeLeft)
            bytesInLane = sizeLeft;
        memcpy(((unsigned char *) &statesAsLanes[laneIndex(instanceIndex, lanePosition)]) + offsetInLane, curData,
            bytesInLane);
        sizeLeft -= bytesInLane;
        lanePosition++;
        curData += bytesInLane;
    ...
    void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    ...
    #define OverWr(argIndex)             \
        STORE256(stateAsLanes[argIndex], \
            LOAD4_64(curData3[argIndex], curData2[argIndex], curData1[argIndex], curData0[argIndex]))

    #define OverWr4(argIndex)                                                                       \
        lanes0 = LOAD256u(curData0[argIndex]), lanes1 = LOAD256u(curData1[argIndex]),               \
        lanes2 = LOAD256u(curData2[argIndex]), lanes3 = LOAD256u(curData3[argIndex]), INTLEAVE(),   \
        STORE256(stateAsLanes[argIndex + 0], lanes0), STORE256(stateAsLanes[argIndex + 1], lanes1), \
        STORE256(stateAsLanes[argIndex + 2], lanes2), STORE256(stateAsLanes[argIndex + 3], lanes3)
...
void KeccakP1600times4_ExtractBytes(
    const void *states, unsigned int instanceIndex, unsigned char *data, unsigned int offset, unsigned int length)
{
    unsigned int sizeLeft = length;
    unsigned int lanePosition = offset / SnP_laneLengthInBytes;
    unsigned int offsetInLane = offset % SnP_laneLengthInBytes;
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        memcpy(curData, ((const unsigned char *) &statesAsLanes[laneIndex(instanceIndex, lanePosition)]) + offsetInLane,
            bytesInLane);
        sizeLeft -= bytesInLane;
        lanePosition++;
        curData += bytesInLane;
...
void KeccakP1600times4_ExtractLanesAll(
    const void *states, unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    // correcting cast-align errors
    ...
    #define Extr(argIndex)                                                                                            \
        curData0[argIndex] = stateAsLanes64[4 * (argIndex)], curData1[argIndex] = stateAsLanes64[4 * (argIndex) + 1], \
        curData2[argIndex] = stateAsLanes64[4 * (argIndex) + 2],                                                      \
        curData3[argIndex] = stateAsLanes64[4 * (argIndex) + 3]

    #define Extr4(argIndex)                                                                                       \
        lanes0 = LOAD256(stateAsLanes[argIndex + 0]), lanes1 = LOAD256(stateAsLanes[argIndex + 1]),               \
        lanes2 = LOAD256(stateAsLanes[argIndex + 2]), lanes3 = LOAD256(stateAsLanes[argIndex + 3]), UNINTLEAVE(), \
        STORE256u(curData0[argIndex], lanes0), STORE256u(curData1[argIndex], lanes1),                             \
        STORE256u(curData2[argIndex], lanes2), STORE256u(curData3[argIndex], lanes3)
...
void KeccakP1600times4_ExtractAndAddBytes(const void *states, unsigned int instanceIndex, const unsigned char *input,
    unsigned char *output, unsigned int offset, unsigned int length)
{
    ...
    while (sizeLeft >= SnP_laneLengthInBytes) {
        ...
        *((UINT64 *) (void *) curOutput) =
            *((const UINT64 *) (const void *) curInput) ^ statesAsLanes[laneIndex(instanceIndex, lanePosition)];
        sizeLeft -= SnP_laneLengthInBytes;
        lanePosition++;
...
void KeccakP1600times4_ExtractAndAddLanesAll(const void *states, const unsigned char *input, unsigned char *output,
    unsigned int laneCount, unsigned int laneOffset)
{
    ...
    #define ExtrXor4(argIndex)                                                                                        \
        lanes0 = LOAD256(stateAsLanes[argIndex + 0]), lanes1 = LOAD256(stateAsLanes[argIndex + 1]),                   \
        lanes2 = LOAD256(stateAsLanes[argIndex + 2]), lanes3 = LOAD256(stateAsLanes[argIndex + 3]), UNINTLEAVE(),     \
        lanesL01 = LOAD256u(curInput0[argIndex]), lanesH01 = LOAD256u(curInput1[argIndex]),                           \
        lanesL23 = LOAD256u(curInput2[argIndex]), lanesH23 = LOAD256u(curInput3[argIndex]),                           \
        XOReq256(lanes0, lanesL01), XOReq256(lanes1, lanesH01), XOReq256(lanes2, lanesL23),                           \
        XOReq256(lanes3, lanesH23), STORE256u(curOutput0[argIndex], lanes0), STORE256u(curOutput1[argIndex], lanes1), \
        STORE256u(curOutput2[argIndex], lanes2), STORE256u(curOutput3[argIndex], lanes3)
...
static ALIGN(KeccakP1600times4_statesAlignment) const UINT64 KeccakF1600RoundConstants[24] = { 0x0000000000000001ULL,
    0x0000000000008082ULL, 0x800000000000808aULL, 0x8000000080008000ULL, 0x000000000000808bULL, 0x0000000080000001ULL,
    0x8000000080008081ULL, 0x8000000000008009ULL, 0x000000000000008aULL, 0x0000000000000088ULL, 0x0000000080008009ULL,
    0x000000008000000aULL, 0x000000008000808bULL, 0x800000000000008bULL, 0x8000000000008089ULL, 0x8000000000008003ULL,
    0x8000000000008002ULL, 0x8000000000000080ULL, 0x000000000000800aULL, 0x800000008000000aULL, 0x8000000080008081ULL,
...
*/
        ...
        #define rounds24                                                                                            \
            prepareTheta thetaRhoPiChiIotaPrepareTheta(0, A, E) thetaRhoPiChiIotaPrepareTheta(1, E, A)              \
                thetaRhoPiChiIotaPrepareTheta(2, A, E) thetaRhoPiChiIotaPrepareTheta(3, E, A)                       \
                    thetaRhoPiChiIotaPrepareTheta(4, A, E) thetaRhoPiChiIotaPrepareTheta(5, E, A)                   \
                        thetaRhoPiChiIotaPrepareTheta(6, A, E) thetaRhoPiChiIotaPrepareTheta(7, E, A)               \
                            thetaRhoPiChiIotaPrepareTheta(8, A, E) thetaRhoPiChiIotaPrepareTheta(9, E, A)           \
                                thetaRhoPiChiIotaPrepareTheta(10, A, E) thetaRhoPiChiIotaPrepareTheta(11, E, A)     \
                                    thetaRhoPiChiIotaPrepareTheta(12, A, E) thetaRhoPiChiIotaPrepareTheta(13, E, A) \
        ...
        #define rounds12                                                                                    \
            prepareTheta thetaRhoPiChiIotaPrepareTheta(12, A, E) thetaRhoPiChiIotaPrepareTheta(13, E, A)    \
                thetaRhoPiChiIotaPrepareTheta(14, A, E) thetaRhoPiChiIotaPrepareTheta(15, E, A)             \
                    thetaRhoPiChiIotaPrepareTheta(16, A, E) thetaRhoPiChiIotaPrepareTheta(17, E, A)         \
                        thetaRhoPiChiIotaPrepareTheta(18, A, E) thetaRhoPiChiIotaPrepareTheta(19, E, A)     \
                            thetaRhoPiChiIotaPrepareTheta(20, A, E) thetaRhoPiChiIotaPrepareTheta(21, E, A) \
                                thetaRhoPiChiIotaPrepareTheta(22, A, E) thetaRhoPiChiIota(23, E, A)

    #elif (Unrolling == 12)
        #define rounds24                                                                                              \
            prepareTheta for (i = 0; i < 24; i += 12)                                                                 \
            {                                                                                                         \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A)                     \
                    thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) thetaRhoPiChiIotaPrepareTheta(i + 3, E, A)             \
                        thetaRhoPiChiIotaPrepareTheta(i + 4, A, E) thetaRhoPiChiIotaPrepareTheta(i + 5, E, A)         \
                            thetaRhoPiChiIotaPrepareTheta(i + 6, A, E) thetaRhoPiChiIotaPrepareTheta(i + 7, E, A)     \
                                thetaRhoPiChiIotaPrepareTheta(i + 8, A, E) thetaRhoPiChiIotaPrepareTheta(i + 9, E, A) \
        ...
        #define rounds12                                                                                      \
            prepareTheta for (i = 12; i < 24; i += 6)                                                         \
            {                                                                                                 \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A)             \
                    thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) thetaRhoPiChiIotaPrepareTheta(i + 3, E, A)     \
                        thetaRhoPiChiIotaPrepareTheta(i + 4, A, E) thetaRhoPiChiIotaPrepareTheta(i + 5, E, A) \
        ...
        #define rounds12                                                                                  \
            prepareTheta for (i = 12; i < 24; i += 4)                                                     \
            {                                                                                             \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A)         \
                    thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) thetaRhoPiChiIotaPrepareTheta(i + 3, E, A) \
            }
    ...
    #elif (Unrolling == 3)
        #define rounds24                                                                          \
            prepareTheta for (i = 0; i < 24; i += 3)                                              \
            {                                                                                     \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
                    thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) copyStateVariables(A, E)           \
            }

        #define rounds12                                                                          \
            prepareTheta for (i = 12; i < 24; i += 3)                                             \
            {                                                                                     \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
                    thetaRhoPiChiIotaPrepareTheta(i + 2, A, E) copyStateVariables(A, E)           \
            }
    ...
    #elif (Unrolling == 2)
        #define rounds24                                                                          \
            prepareTheta for (i = 0; i < 24; i += 2)                                              \
            {                                                                                     \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
            }

        #define rounds12                                                                          \
            prepareTheta for (i = 12; i < 24; i += 2)                                             \
            {                                                                                     \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
            }
    ...
    #elif (Unrolling == 1)
        #define rounds24                                                        \
            prepareTheta for (i = 0; i < 24; i++)                               \
            {                                                                   \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) \
            }

        #define rounds12                                                        \
            prepareTheta for (i = 12; i < 24; i++)                              \
            {                                                                   \
                thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E) \
            }
    ...
    #define roundsN(__nrounds)                                                                \
        prepareTheta i = 24 - (__nrounds);                                                    \
        if ((i & 1) != 0) {                                                                   \
            thetaRhoPiChiIotaPrepareTheta(i, A, E) copyStateVariables(A, E)++ i;              \
        }                                                                                     \
        for (/* empty */; i < 24; i += 2) {                                                   \
            thetaRhoPiChiIotaPrepareTheta(i, A, E) thetaRhoPiChiIotaPrepareTheta(i + 1, E, A) \
        }
...
size_t KeccakF1600times4_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel,
    unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
    #if 0
        ...
        declareABCDE
            ...
            copyFromState(A, statesAsLanes) while (dataByteLen >= (laneOffsetParallel * 3 + laneCount) * 8)
        {
        #define XOR_In(Xxx, argIndex) \
            ...
            XOR_In(Asa, 20);
        #undef XOR_In
            rounds24 curData0 += laneOffsetSerial;
            curData1 += laneOffsetSerial;
            curData2 += laneOffsetSerial;
            curData3 += laneOffsetSerial;
            dataByteLen -= laneOffsetSerial * 8;
        }
        copyToState(statesAsLanes, A) return (const unsigned char *) curData0 - dataStart;
    #endif
    } else {
        //        unsigned int i;
        const unsigned char *dataStart = data;
    ...
    unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
        ...
        #define XOR_In(Xxx, argIndex) \
            ...
            XOR_In(Asa, 20);
        #undef XOR_In
            rounds12 curData0 += laneOffsetSerial;
            curData1 += laneOffsetSerial;
            curData2 += laneOffsetSerial;
            curData3 += laneOffsetSerial;
            dataByteLen -= laneOffsetSerial * 8;
