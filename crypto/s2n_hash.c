    ...
    switch (alg) {
        ...
        case S2N_HASH_SHA512:
            *out = SHA512_DIGEST_LENGTH;
            break;
        case S2N_HASH_MD5_SHA1:
            *out = MD5_DIGEST_LENGTH + SHA_DIGEST_LENGTH;
            break;
        default:
...
static int s2n_low_level_hash_update(struct s2n_hash_state *state, const void *data, uint32_t size)
{
    ...
    switch (state->alg) {
        ...
        case S2N_HASH_MD5_SHA1:
            POSIX_GUARD_OSSL(
                SHA1_Update(&state->digest.low_level.md5_sha1.sha1, data, size), S2N_ERR_HASH_UPDATE_FAILED);
...
static int s2n_evp_hash_init(struct s2n_hash_state *state, s2n_hash_algorithm alg)
{
    ...
    if (alg == S2N_HASH_MD5_SHA1 && s2n_use_custom_md5_sha1()) {
        ...
        POSIX_GUARD_OSSL(
            EVP_DigestInit_ex(state->digest.high_level.evp.ctx, EVP_sha1(), NULL), S2N_ERR_HASH_INIT_FAILED);
...
static int s2n_evp_hash_update(struct s2n_hash_state *state, const void *data, uint32_t size)
{
    ...
    if (state->alg == S2N_HASH_MD5_SHA1 && s2n_use_custom_md5_sha1()) {
        ...
        POSIX_GUARD_OSSL(
            EVP_DigestUpdate(state->digest.high_level.evp_md5_secondary.ctx, data, size), S2N_ERR_HASH_UPDATE_FAILED);
    }
...
static int s2n_evp_hash_digest(struct s2n_hash_state *state, void *out, uint32_t size)
{
    ...
    POSIX_GUARD_OSSL(
        EVP_DigestFinal_ex(state->digest.high_level.evp.ctx, out, &digest_size), S2N_ERR_HASH_DIGEST_FAILED);
    return S2N_SUCCESS;
}
...
static int s2n_evp_hash_copy(struct s2n_hash_state *to, struct s2n_hash_state *from)
{
    ...
    POSIX_GUARD_OSSL(
        EVP_MD_CTX_copy_ex(to->digest.high_level.evp.ctx, from->digest.high_level.evp.ctx), S2N_ERR_HASH_COPY_FAILED);
...
static int s2n_evp_hash_reset(struct s2n_hash_state *state)
{
    ...
    if (state->alg == S2N_HASH_MD5_SHA1 && s2n_use_custom_md5_sha1()) {
        POSIX_GUARD_OSSL(
            S2N_EVP_MD_CTX_RESET(state->digest.high_level.evp_md5_secondary.ctx), S2N_ERR_HASH_WIPE_FAILED);
    }
