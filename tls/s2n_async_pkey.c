...
static S2N_RESULT s2n_async_pkey_op_set_output_decrypt(
    struct s2n_async_pkey_op *op, const uint8_t *data, uint32_t data_len);
...
static S2N_RESULT s2n_async_pkey_op_set_output_sign(
    struct s2n_async_pkey_op *op, const uint8_t *data, uint32_t data_len);
    ...
    struct s2n_hash_state *digest, struct s2n_blob *signature)
{
    ...
    RESULT_GUARD_POSIX(s2n_asn1der_to_public_key_and_type(
        &public_key, &pkey_type, &conn->handshake_params.our_chain_and_key->cert_chain->head->raw));
