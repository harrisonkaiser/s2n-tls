...
S2N_RESULT s2n_cipher_suites_cleanup(void)
{
    ...
    for (int i = 0; i < num_cipher_suites; i++) {
        ...
        if (cur_suite->sslv3_cipher_suite != cur_suite) {
            RESULT_GUARD_POSIX(
                s2n_free_object((uint8_t **) &cur_suite->sslv3_cipher_suite, sizeof(struct s2n_cipher_suite)));
        }
...
int s2n_set_cipher_as_client(struct s2n_connection *conn, uint8_t wire[S2N_TLS_CIPHER_SUITE_LEN])
{
    ...
    if (conn->psk_params.chosen_psk) {
        POSIX_ENSURE(cipher_suite->prf_alg == conn->psk_params.chosen_psk->hmac_alg, S2N_ERR_CIPHER_NOT_SUPPORTED);
    }
...
static int s2n_wire_ciphers_contain(
    const uint8_t *match, const uint8_t *wire, uint32_t count, uint32_t cipher_suite_len)
{
