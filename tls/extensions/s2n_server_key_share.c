...
int s2n_server_key_share_send_check_pq_hybrid(struct s2n_connection *conn)
{
    ...
    POSIX_ENSURE(
        client_params->kem_params.public_key.size == server_kem_group->kem->public_key_length, S2N_ERR_BAD_KEY_SHARE);
...
static int s2n_server_key_share_recv_pq_hybrid(
    struct s2n_connection *conn, uint16_t named_group_iana, struct s2n_stuffer *extension)
{
    POSIX_ENSURE_REF(conn);
    ...
    POSIX_ENSURE(
        s2n_kem_preferences_includes_tls13_kem_group(kem_pref, named_group_iana), S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
    ...
    POSIX_ENSURE(
        received_total_share_size == server_kem_group_params->kem_group->server_share_size, S2N_ERR_BAD_KEY_SHARE);
    ...
    POSIX_ENSURE(
        s2n_ecc_evp_read_params_point(extension, ecc_share_size, &point_blob) == S2N_SUCCESS, S2N_ERR_BAD_KEY_SHARE);
    POSIX_ENSURE(s2n_ecc_evp_parse_params_point(&point_blob, &server_kem_group_params->ecc_params) == S2N_SUCCESS,
    ...
    POSIX_ENSURE(
        s2n_kem_recv_ciphertext(extension, &conn->kex_params.client_kem_group_params.kem_params) == S2N_SUCCESS,
        S2N_ERR_BAD_KEY_SHARE);

    return S2N_SUCCESS;
}

static int s2n_server_key_share_recv_ecc(
    struct s2n_connection *conn, uint16_t named_group_iana, struct s2n_stuffer *extension)
{
    POSIX_ENSURE_REF(conn);
    POSIX_ENSURE_REF(extension);
    ...
    /* This check should have been done higher up, but including it here as well for extra defense. */
    POSIX_ENSURE(s2n_ecc_preferences_includes_curve(ecc_pref, named_group_iana), S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
    ...
    if (s2n_is_hello_retry_handshake(conn) && !s2n_is_hello_retry_message(conn)) {
        ...
        POSIX_ENSURE(negotiated_curve == previous_negotiated_curve, S2N_ERR_BAD_MESSAGE);
    }
    ...
    POSIX_ENSURE(
        client_ecc_evp_params->negotiated_curve == server_ecc_evp_params->negotiated_curve, S2N_ERR_BAD_KEY_SHARE);
...
int s2n_extensions_server_key_share_select(struct s2n_connection *conn)
{
    ...
    if (conn->kex_params.client_kem_group_params.kem_group) {
        ...
        conn->kex_params.server_kem_group_params.kem_params.kem =
            conn->kex_params.client_kem_group_params.kem_params.kem;
    ...
    if (conn->kex_params.client_ecc_evp_params.negotiated_curve) {
        conn->kex_params.server_ecc_evp_params.negotiated_curve =
            conn->kex_params.client_ecc_evp_params.negotiated_curve;
