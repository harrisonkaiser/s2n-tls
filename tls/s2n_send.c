...
ssize_t s2n_sendv_with_offset_impl(
    struct s2n_connection *conn, const struct iovec *bufs, ssize_t count, ssize_t offs, s2n_blocked_status *blocked)
{
    ...
    *blocked = S2N_BLOCKED_ON_WRITE;
    ...
    while (total_size - conn->current_user_data_consumed) {
        ...
        int written_to_record = s2n_record_writev(
            conn, TLS_APPLICATION_DATA, bufs, count, conn->current_user_data_consumed + offs, to_write);
