...
static int s2n_handshake_get_hash_state_ptr(
...
S2N_RESULT s2n_handshake_copy_hash_state(
    struct s2n_connection *conn, s2n_hash_algorithm hash_alg, struct s2n_hash_state *copy)
{
    struct s2n_hash_state *hash_state = NULL;
    RESULT_GUARD_POSIX(s2n_handshake_get_hash_state_ptr(conn, hash_alg, &hash_state));
...
static int s2n_find_cert_matches(struct s2n_map *domain_name_to_cert_map, struct s2n_blob *dns_name,
    struct s2n_cert_chain_and_key *matches[S2N_CERT_TYPE_COUNT], uint8_t *match_exists)
{
...
int s2n_conn_find_name_matching_certs(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_find_cert_matches(conn->config->domain_name_to_cert_map, &normalized_name,
        conn->handshake_params.exact_sni_matches, &(conn->handshake_params.exact_sni_match_exists)));
    ...
    if (!conn->handshake_params.exact_sni_match_exists) {
        ...
        POSIX_GUARD(s2n_find_cert_matches(conn->config->domain_name_to_cert_map, &wildcard_blob,
            conn->handshake_params.wc_sni_matches, &(conn->handshake_params.wc_sni_match_exists)));
    }
     ...
     */
    conn->server_name_used = conn->server_name_used || conn->handshake_params.exact_sni_match_exists
