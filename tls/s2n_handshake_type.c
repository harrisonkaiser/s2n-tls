...
{
    return conn && s2n_connection_get_protocol_version(conn) < S2N_TLS13 && (conn->handshake.handshake_type & flag);
}
...
{
    return s2n_connection_get_protocol_version(conn) >= S2N_TLS13 && (conn->handshake.handshake_type & flag);
}
