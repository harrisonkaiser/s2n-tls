...
#define S2N_TLS12_MAX_RECORD_LEN_FOR(frag) ((frag) + S2N_TLS12_ENCRYPTION_OVERHEAD_SIZE + S2N_TLS_RECORD_HEADER_LENGTH)
...
#define S2N_TLS13_MAX_RECORD_LEN_FOR(frag) \
    ((frag) + S2N_TLS_CONTENT_TYPE_LENGTH + S2N_TLS13_ENCRYPTION_OVERHEAD_SIZE + S2N_TLS_RECORD_HEADER_LENGTH)
...
S2N_RESULT s2n_record_max_write_size(
    struct s2n_connection *conn, uint16_t max_fragment_size, uint16_t *max_record_size);
...
extern int s2n_record_writev(struct s2n_connection *conn, uint8_t content_type, const struct iovec *in, int in_count,
...
extern int s2n_sslv2_record_header_parse(
...
extern S2N_RESULT s2n_aead_aad_init(const struct s2n_connection *conn, uint8_t *sequence_number, uint8_t content_type,
    uint16_t record_length, struct s2n_blob *ad);
