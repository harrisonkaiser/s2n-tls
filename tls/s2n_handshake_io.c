...
{
    return (conn != NULL && s2n_result_is_ok(s2n_handshake_validate(&(conn->handshake)))
...
static S2N_RESULT s2n_conn_set_tls13_handshake_type(struct s2n_connection *conn)
{
    ...
    if (conn->mode == S2N_CLIENT && client_cert_auth_type == S2N_CERT_AUTH_REQUIRED && IS_FULL_HANDSHAKE(conn)) {
    ...
    } else if (conn->mode == S2N_SERVER && client_cert_auth_type != S2N_CERT_AUTH_NONE && IS_FULL_HANDSHAKE(conn)) {
...
static int s2n_read_full_handshake_message(struct s2n_connection *conn, uint8_t *message_type)
{
    ...
    if (current_handshake_data < TLS_HANDSHAKE_HEADER_LENGTH) {
        ...
        POSIX_GUARD(
            s2n_stuffer_copy(&conn->in, &conn->handshake.io, (TLS_HANDSHAKE_HEADER_LENGTH - current_handshake_data)));
    }
