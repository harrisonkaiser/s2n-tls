...
static int s2n_config_update_domain_name_to_cert_map(
    struct s2n_config *config, struct s2n_blob *name, struct s2n_cert_chain_and_key *cert_key_pair)
{
...
static int s2n_config_build_domain_name_to_cert_map(
...
int s2n_config_init_session_ticket_keys(struct s2n_config *config)
{
    ...
    if (config->ticket_key_hashes == NULL) {
        POSIX_ENSURE_REF(
            config->ticket_key_hashes = s2n_set_new(SHA_DIGEST_LENGTH, s2n_verify_unique_ticket_key_comparator));
    }
...
static int s2n_config_add_cert_chain_and_key_impl(
    struct s2n_config *config, struct s2n_cert_chain_and_key *cert_key_pair)
{
...
int s2n_config_set_cache_store_callback(
...
int s2n_config_set_cache_retrieve_callback(
...
int s2n_config_set_cache_delete_callback(
...
int s2n_config_set_extension_data(
...
int s2n_config_set_client_hello_cb_mode(struct s2n_config *config, s2n_client_hello_cb_mode cb_mode)
{
    POSIX_ENSURE_REF(config);
    POSIX_ENSURE(
        cb_mode == S2N_CLIENT_HELLO_CB_BLOCKING || cb_mode == S2N_CLIENT_HELLO_CB_NONBLOCKING, S2N_ERR_INVALID_STATE);
