...
int s2n_server_key_recv(struct s2n_connection *conn)
{
    ...
    S2N_ERROR_IF(s2n_pkey_verify(
                     &conn->handshake_params.server_public_key, active_sig_scheme->sig_alg, signature_hash, &signature)
            < 0,
        S2N_ERR_BAD_MESSAGE);
...
int s2n_ecdhe_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *data_to_verify, struct s2n_kex_raw_server_data *raw_server_data)
{
    struct s2n_stuffer *in = &conn->handshake.io;
...
int s2n_dhe_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *data_to_verify, struct s2n_kex_raw_server_data *raw_server_data)
{
    struct s2n_stuffer *in = &conn->handshake.io;
...
int s2n_kem_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *data_to_verify, struct s2n_kex_raw_server_data *raw_server_data)
{
...
int s2n_kem_server_key_recv_parse_data(struct s2n_connection *conn, struct s2n_kex_raw_server_data *raw_server_data)
{
    ...
    S2N_ERROR_IF(
        kem_data->raw_public_key.size != conn->kex_params.kem_params.kem->public_key_length, S2N_ERR_BAD_MESSAGE);

    return 0;
}

int s2n_hybrid_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *total_data_to_verify, struct s2n_kex_raw_server_data *raw_server_data)
{
    POSIX_ENSURE_REF(conn);
    POSIX_ENSURE_REF(conn->secure);
    POSIX_ENSURE_REF(conn->secure->cipher_suite);
...
int s2n_server_key_send(struct s2n_connection *conn)
{
    ...
    S2N_ASYNC_PKEY_SIGN(
        conn, conn->handshake_params.conn_sig_scheme.sig_alg, signature_hash, s2n_server_key_send_write_signature);
}
