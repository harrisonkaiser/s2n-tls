...
#define IS_NEGOTIATED(conn) (s2n_handshake_type_check_flag(conn, NEGOTIATED))

#define IS_FULL_HANDSHAKE(conn) (s2n_handshake_type_check_flag(conn, FULL_HANDSHAKE))

#define IS_RESUMPTION_HANDSHAKE(conn) (!IS_FULL_HANDSHAKE(conn) && IS_NEGOTIATED(conn))

#define IS_CLIENT_AUTH_HANDSHAKE(conn) (s2n_handshake_type_check_flag(conn, CLIENT_AUTH))
...
#define IS_OCSP_STAPLED(conn) (s2n_handshake_type_check_tls12_flag(conn, OCSP_STATUS))

#define IS_ISSUING_NEW_SESSION_TICKET(conn) (s2n_handshake_type_check_tls12_flag(conn, WITH_SESSION_TICKET))

#define IS_NPN_HANDSHAKE(conn) (s2n_handshake_type_check_tls12_flag(conn, WITH_NPN))

#define IS_HELLO_RETRY_HANDSHAKE(conn) (s2n_handshake_type_check_tls13_flag(conn, HELLO_RETRY_REQUEST))

#define IS_MIDDLEBOX_COMPAT_MODE(conn) (s2n_handshake_type_check_tls13_flag(conn, MIDDLEBOX_COMPAT))

#define WITH_EARLY_DATA(conn) (s2n_handshake_type_check_tls13_flag(conn, WITH_EARLY_DATA))

#define WITH_EARLY_CLIENT_CCS(conn) (s2n_handshake_type_check_tls13_flag(conn, EARLY_CLIENT_CCS))
