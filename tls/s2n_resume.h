...
#define S2N_TLS12_TICKET_SIZE_IN_BYTES \
...
#define S2N_TLS12_SESSION_SIZE \
    S2N_STATE_FORMAT_LEN + S2N_SESSION_TICKET_SIZE_LEN + S2N_TLS12_TICKET_SIZE_IN_BYTES + S2N_TLS12_STATE_SIZE_IN_BYTES
...
extern struct s2n_ticket_key *s2n_find_ticket_key(
    struct s2n_config *config, const uint8_t name[S2N_TICKET_KEY_NAME_LEN]);
...
typedef enum { S2N_STATE_WITH_SESSION_ID = 0, S2N_STATE_WITH_SESSION_TICKET } s2n_client_tls_session_state_format;
