...
static int s2n_evp_pkey_p_hash_digest_init(struct s2n_prf_working_space *ws)
{
    ...
    POSIX_GUARD_OSSL(EVP_DigestSignInit(ws->p_hash.evp_hmac.evp_digest.ctx, NULL, ws->p_hash.evp_hmac.evp_digest.md,
                         NULL, ws->p_hash.evp_hmac.ctx.evp_pkey),
...
static int s2n_evp_pkey_p_hash_init(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg, struct s2n_blob *secret)
{
    ...
    POSIX_ENSURE_REF(
        ws->p_hash.evp_hmac.ctx.evp_pkey = EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, NULL, secret->data, secret->size));
...
{
    POSIX_GUARD_OSSL(
        EVP_DigestSignUpdate(ws->p_hash.evp_hmac.evp_digest.ctx, data, (size_t) size), S2N_ERR_P_HASH_UPDATE_FAILED);
...
static int s2n_evp_pkey_p_hash_final(struct s2n_prf_working_space *ws, void *digest, uint32_t size)
{
    ...
    POSIX_GUARD_OSSL(EVP_DigestSignFinal(ws->p_hash.evp_hmac.evp_digest.ctx, (unsigned char *) digest, &digest_size),
...
static int s2n_evp_hmac_p_hash_init(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg, struct s2n_blob *secret)
{
    ...
    POSIX_GUARD_OSSL(HMAC_Init_ex(ws->p_hash.evp_hmac.ctx.hmac_ctx, secret->data, secret->size,
                         ws->p_hash.evp_hmac.evp_digest.md, NULL),
        S2N_ERR_P_HASH_INIT_FAILED);
    return S2N_SUCCESS;
}
...
static int s2n_evp_hmac_p_hash_final(struct s2n_prf_working_space *ws, void *digest, uint32_t size)
{
    ...
    POSIX_GUARD_OSSL(HMAC_Final(ws->p_hash.evp_hmac.ctx.hmac_ctx, (unsigned char *) digest, &digest_size),
        S2N_ERR_P_HASH_FINAL_FAILED);
    return S2N_SUCCESS;
}
...
static int s2n_evp_hmac_p_hash_reset(struct s2n_prf_working_space *ws)
{
    ...
    POSIX_GUARD_OSSL(HMAC_Init_ex(ws->p_hash.evp_hmac.ctx.hmac_ctx, NULL, 0, ws->p_hash.evp_hmac.evp_digest.md, NULL),
        S2N_ERR_P_HASH_INIT_FAILED);
    return S2N_SUCCESS;
}
...
static int s2n_p_hash(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg, struct s2n_blob *secret,
    struct s2n_blob *label, struct s2n_blob *seed_a, struct s2n_blob *seed_b, struct s2n_blob *seed_c,
    struct s2n_blob *out)
{
    uint8_t digest_size;
...
static int s2n_prf(struct s2n_connection *conn, struct s2n_blob *secret, struct s2n_blob *label,
    struct s2n_blob *seed_a, struct s2n_blob *seed_b, struct s2n_blob *seed_c, struct s2n_blob *out)
{
    ...
    if (conn->actual_protocol_version == S2N_TLS12) {
        return s2n_p_hash(
            conn->prf_space, conn->secure->cipher_suite->prf_alg, secret, label, seed_a, seed_b, seed_c, out);
    }
...
int s2n_tls_prf_master_secret(struct s2n_connection *conn, struct s2n_blob *premaster_secret)
{
    ...
    struct s2n_blob client_random = { .size = sizeof(conn->handshake_params.client_random),
        .data = conn->handshake_params.client_random };
    struct s2n_blob server_random = { .size = sizeof(conn->handshake_params.server_random),
        .data = conn->handshake_params.server_random };
    struct s2n_blob master_secret = { .size = sizeof(conn->secrets.tls12.master_secret),
        .data = conn->secrets.tls12.master_secret };
...
int s2n_hybrid_prf_master_secret(struct s2n_connection *conn, struct s2n_blob *premaster_secret)
{
    ...
    return s2n_prf(conn, premaster_secret, &label, &client_random, &server_random,
        &conn->kex_params.client_key_exchange_message, &master_secret);
}
 ...
 *#                    [0..47];
 */
S2N_RESULT s2n_tls_prf_extended_master_secret(struct s2n_connection *conn, struct s2n_blob *premaster_secret,
    struct s2n_blob *session_hash, struct s2n_blob *sha1_hash)
{
    RESULT_ENSURE_REF(conn);

    struct s2n_blob extended_master_secret = { .size = sizeof(conn->secrets.tls12.master_secret),
        .data = conn->secrets.tls12.master_secret };
...
S2N_RESULT s2n_prf_get_digest_for_ems(
    struct s2n_connection *conn, struct s2n_blob *message, s2n_hash_algorithm hash_alg, struct s2n_blob *output)
{
    RESULT_ENSURE_REF(conn);
...
static int s2n_sslv3_finished(
    struct s2n_connection *conn, uint8_t prefix[4], struct s2n_hash_state *hash_workspace, uint8_t *out)
{
    POSIX_ENSURE_REF(conn);
    POSIX_ENSURE_REF(conn->handshake.hashes);

    uint8_t xorpad1[48] = { 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    ...
    uint8_t xorpad2[48] = { 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c,
...
static int s2n_prf_make_client_key(struct s2n_connection *conn, struct s2n_stuffer *key_material)
{
    ...
    if (conn->mode == S2N_CLIENT) {
        POSIX_GUARD(
            conn->secure->cipher_suite->record_alg->cipher->set_encryption_key(&conn->secure->client_key, &client_key));
    } else {
        POSIX_GUARD(
            conn->secure->cipher_suite->record_alg->cipher->set_decryption_key(&conn->secure->client_key, &client_key));
    }
...
static int s2n_prf_make_server_key(struct s2n_connection *conn, struct s2n_stuffer *key_material)
{
    ...
    if (conn->mode == S2N_SERVER) {
        POSIX_GUARD(
            conn->secure->cipher_suite->record_alg->cipher->set_encryption_key(&conn->secure->server_key, &server_key));
    } else {
        POSIX_GUARD(
            conn->secure->cipher_suite->record_alg->cipher->set_decryption_key(&conn->secure->server_key, &server_key));
    }
...
int s2n_prf_key_expansion(struct s2n_connection *conn)
{
    ...
    struct s2n_blob client_random = { .data = conn->handshake_params.client_random,
        .size = sizeof(conn->handshake_params.client_random) };
    struct s2n_blob server_random = { .data = conn->handshake_params.server_random,
        .size = sizeof(conn->handshake_params.server_random) };
    struct s2n_blob master_secret = { .data = conn->secrets.tls12.master_secret,
        .size = sizeof(conn->secrets.tls12.master_secret) };
    ...
    POSIX_GUARD(s2n_hmac_init(&conn->secure->client_record_mac, conn->secure->cipher_suite->record_alg->hmac_alg,
        client_mac_write_key, mac_size));
    ...
    POSIX_GUARD(s2n_hmac_init(&conn->secure->server_record_mac, conn->secure->cipher_suite->record_alg->hmac_alg,
        server_mac_write_key, mac_size));
     ...
     */
    if (conn->secure->cipher_suite->record_alg->cipher->type == S2N_COMPOSITE) {
        POSIX_GUARD(conn->secure->cipher_suite->record_alg->cipher->io.comp.set_mac_write_key(
            &conn->secure->server_key, server_mac_write_key, mac_size));
        POSIX_GUARD(conn->secure->cipher_suite->record_alg->cipher->io.comp.set_mac_write_key(
            &conn->secure->client_key, client_mac_write_key, mac_size));
    }
