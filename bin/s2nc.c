...
void usage()
{
    ...
    fprintf(
        stderr, "    Sets the SNI server name header for this client.  If not specified, the host value is used.\n");
    fprintf(stderr, "\n");
    ...
    fprintf(stderr,
        "    Location of trust store CA file (PEM format). If neither -f or -d are specified. System defaults will be "
        "used.\n");
...
int main(int argc, char *const *argv)
{
    ...
    do {
        ...
        if (client_cert_input) {
            ...
            GUARD_EXIT(
                s2n_config_add_cert_chain_and_key_to_store(config, chain_and_key), "Error setting certificate/key");
        }
        ...
        GUARD_EXIT(
            s2n_connection_set_client_auth_type(conn, S2N_CERT_AUTH_OPTIONAL), "Error setting ClientAuth optional");
        ...
        if (ticket_in) {
            ...
            GUARD_EXIT(
                load_file_to_array(ticket_in, session_state, session_state_length), "Failed to read ticket-in file");
        }
        ...
        if (early_data) {
            ...
            if (session_state_length) {
                ...
                GUARD_EXIT(
                    early_data_send(conn, (uint8_t *) early_data, early_data_length), "Error sending early data");
            }
        }
