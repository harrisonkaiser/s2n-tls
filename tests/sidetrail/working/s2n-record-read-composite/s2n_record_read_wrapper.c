...
int s2n_record_parse_composite(const struct s2n_cipher_suite *cipher_suite, struct s2n_connection *conn,
...
int decrypt_comp(struct s2n_session_key *session_key, struct s2n_blob *iv, struct s2n_blob *in, struct s2n_blob *out)
...
int s2n_record_parse_wrapper(
    int *xor_pad, int *digest_pad, int padding_length, int encrypted_length, uint8_t content_type)
{
    ...
    return s2n_record_parse_composite(
