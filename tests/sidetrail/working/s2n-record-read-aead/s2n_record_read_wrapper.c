...
int s2n_record_parse_aead(const struct s2n_cipher_suite *cipher_suite, struct s2n_connection *conn,
    uint8_t content_type, uint16_t encrypted_length, uint8_t *implicit_iv, struct s2n_hmac_state *mac,
...
int decrypt_aead(struct s2n_session_key *session_key, struct s2n_blob *iv, struct s2n_blob *aad, struct s2n_blob *in,
    struct s2n_blob *out)
...
int s2n_record_parse_wrapper(
    int *xor_pad, int *digest_pad, int padding_length, int encrypted_length, uint8_t content_type, int flags)
{
    ...
    struct s2n_hmac_state hmac = { .alg = S2N_HMAC_SHA1,
    ...
    struct s2n_record_algorithm record_algorithm = { .cipher = &aead_cipher, .flags = flags };
    ...
    return s2n_record_parse_aead(
        &cipher_suite, &conn, content_type, encrypted_length, implicit_iv, &hmac, sequence_number, &session_key);
}
