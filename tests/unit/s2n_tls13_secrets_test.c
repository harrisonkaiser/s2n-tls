    ...
    /* Test: s2n_tls13_derive_secret */
    {
        ...
        /* Safety */
        {
            ...
            EXPECT_ERROR_WITH_ERRNO(
                s2n_tls13_derive_secret(&empty_conn, S2N_EARLY_SECRET, S2N_CLIENT, &blob), S2N_ERR_NULL);
        ...
        /* Fails if correct transcript digest not available */
        {
            ...
            EXPECT_ERROR_WITH_ERRNO(
                s2n_tls13_derive_secret(conn, S2N_MASTER_SECRET, S2N_SERVER, &output), S2N_ERR_SECRET_SCHEDULE_STATE);
        ...
        for (size_t i = 0; i < test_cases_count; i++) {
            ...
            EXPECT_OK(s2n_set_test_key_shares(conn, test_cases[i].curve));
            EXPECT_OK(
                s2n_tls13_derive_secret(conn, test_cases[i].next_secret_type, test_cases[i].secret_mode, &output));
            EXPECT_BYTEARRAY_NOT_EQUAL(output.data, empty_secret, sizeof(empty_secret));
        }
    }
    ...
    /* s2n_tls13_secrets_clean */
    {
        ...
        /* Wipes all secrets */
        {
            ...
            EXPECT_BYTEARRAY_NOT_EQUAL(
                conn->secrets.tls13.resumption_master_secret, empty_secret, sizeof(empty_secret));
...
/* s2n_tls13_secrets_update */
{
    ...
    /* Derives early secret on CLIENT_HELLO */
    {
        ...
        /* Clear secret */
        EXPECT_MEMCPY_SUCCESS(conn->secrets.tls13.client_early_secret, empty_secret, sizeof(empty_secret));
    ...
    /* Computes finished keys on SERVER_HELLO */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(conn->handshake.client_finished, empty_secret, sizeof(empty_secret));
        EXPECT_BYTEARRAY_EQUAL(conn->handshake.server_finished, empty_secret, sizeof(empty_secret));
        ...
        EXPECT_BYTEARRAY_NOT_EQUAL(conn->handshake.client_finished, empty_secret, sizeof(empty_secret));
        EXPECT_BYTEARRAY_NOT_EQUAL(conn->handshake.server_finished, empty_secret, sizeof(empty_secret));
    }
    ...
    /* Derives application secrets on SERVER_FINISHED */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.client_app_secret, empty_secret, sizeof(empty_secret));
        EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.server_app_secret, empty_secret, sizeof(empty_secret));
    ...
    /* Derives resumption secret on CLIENT_FINISHED */
    {
        ...
        conn->actual_protocol_version = S2N_TLS13;
        EXPECT_OK(s2n_connection_set_test_master_secret(conn, &test_secret));
        EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.resumption_master_secret, empty_secret, sizeof(empty_secret));
