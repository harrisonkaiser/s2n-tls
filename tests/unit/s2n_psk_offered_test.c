...
/* Test: s2n_validate_ticket_lifetime */
{
    ...
    EXPECT_ERROR_WITH_ERRNO(
        s2n_validate_ticket_lifetime(conn, obfuscated_ticket_age, ticket_age_add), S2N_ERR_INVALID_SESSION_TICKET);
...
/* Test: s2n_offered_psk_list_choose_psk */
{
    ...
    /* Resumption psk has expired */
    {
        ...
        struct s2n_offered_psk client_psk = {
            .identity = psk_identity.blob, .wire_index = wire_index, .obfuscated_ticket_age = 100
        };
        server_conn->config->session_state_lifetime_in_nanos = 0;

        EXPECT_FAILURE_WITH_ERRNO(
            s2n_offered_psk_list_choose_psk(&identity_list, &client_psk), S2N_ERR_INVALID_SESSION_TICKET);
...
/* Functional test: Process the output of sending the psk extension */
{
    ...
    EXPECT_SUCCESS(
        s2n_stuffer_write_bytes(&identity_list.wire_data, s2n_stuffer_raw_read(&conn->handshake.io, size), size));
