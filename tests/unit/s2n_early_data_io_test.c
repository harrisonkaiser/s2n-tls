...
/* Early data rejected, but too much early data received */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_negotiate_test_server_and_client(server_conn, client_conn), S2N_ERR_MAX_EARLY_DATA_SIZE);
...
EXPECT_FAILURE_WITH_ERRNO(
    s2n_send(client_conn, test_data, sizeof(test_data), &blocked), S2N_ERR_EARLY_DATA_NOT_ALLOWED);
EXPECT_FAILURE_WITH_ERRNO(s2n_send(client_conn, test_data, 0, &blocked), S2N_ERR_EARLY_DATA_NOT_ALLOWED);
...
EXPECT_OK(s2n_negotiate_test_server_and_client_until_message(server_conn, client_conn, SERVER_FINISHED));
...
EXPECT_FAILURE_WITH_ERRNO(
    s2n_send(client_conn, test_data, TEST_MAX_EARLY_DATA_SIZE, &blocked), S2N_ERR_MAX_EARLY_DATA_SIZE);
...
/* End early data before the server accepts the early data request. */
{
    ...
    EXPECT_ERROR_WITH_ERRNO(
        s2n_negotiate_test_server_and_client_until_message(server_conn, client_conn, CLIENT_FINISHED),
        S2N_ERR_EARLY_DATA_BLOCKED);
    EXPECT_EQUAL(s2n_conn_get_current_message_type(client_conn), CLIENT_FINISHED);
...
/* Test s2n_recv with early data */
{
    /* s2n_recv can read early data sent with s2n_send */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_recv(server_conn, test_buffer, sizeof(test_buffer), &blocked), S2N_ERR_EARLY_DATA_NOT_ALLOWED);
        ...
        EXPECT_SUCCESS_S2N_RECV(server_conn, test_buffer, sizeof(test_buffer), &blocked, test_data, sizeof(test_data));
        ...
        EXPECT_SUCCESS_S2N_RECV(
            server_conn, test_buffer, (sizeof(test_data) - 2), &blocked, (test_data + 2), (sizeof(test_data) - 2));
    ...
    /* s2n_recv fails if it encounters a handshake message instead of early data. */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_recv(server_conn, test_buffer, sizeof(test_buffer), &blocked), S2N_ERR_BAD_MESSAGE);
    ...
    /* s2n_recv fails on too much early data */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(s2n_negotiate(server_conn, &blocked), S2N_ERR_EARLY_DATA_BLOCKED);
        EXPECT_SUCCESS_S2N_RECV(
            server_conn, test_buffer, sizeof(test_buffer), &blocked, large_test_data, TEST_MAX_EARLY_DATA_SIZE);
        ...
        EXPECT_FAILURE_WITH_ERRNO(s2n_negotiate(server_conn, &blocked), S2N_ERR_EARLY_DATA_BLOCKED);
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_recv(server_conn, test_buffer, sizeof(test_buffer), &blocked), S2N_ERR_MAX_EARLY_DATA_SIZE);
