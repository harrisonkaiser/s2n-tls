...
int s2n_negotiate_nonblocking_ch_cb(
    struct s2n_connection *conn, struct client_hello_context *ch_ctx, bool server_name_used)
{
    s2n_blocked_status blocked;
...
int s2n_negotiate_nonblocking_poll(struct s2n_connection *conn, struct client_hello_context *ch_ctx)
{
    EXPECT_NOT_NULL(conn);
...
int init_server_conn(struct s2n_connection **conn, struct s2n_test_io_pair *io_pair, struct s2n_config *config)
{
...
int start_client_conn(struct s2n_test_io_pair *io_pair, pid_t *pid, int expect_failure, int expect_server_name_used)
{
    /* Create a pipe */
...
static int test_case_clean(struct s2n_connection *conn, pid_t client_pid, struct s2n_config *config,
    struct s2n_test_io_pair *io_pair, struct client_hello_context *ch_ctx)
{
    s2n_blocked_status blocked;
    int status;
...
int run_test_config_swap_ch_cb(
    s2n_client_hello_cb_mode cb_mode, struct s2n_cert_chain_and_key *chain_and_key, struct client_hello_context *ch_ctx)
{
...
int run_test_no_config_swap_ch_cb(
    s2n_client_hello_cb_mode cb_mode, struct s2n_cert_chain_and_key *chain_and_key, struct client_hello_context *ch_ctx)
{
    struct s2n_test_io_pair io_pair;
    struct s2n_config *config;
...
int run_test_reject_handshake_ch_cb(
    s2n_client_hello_cb_mode cb_mode, struct s2n_cert_chain_and_key *chain_and_key, struct client_hello_context *ch_ctx)
{
    struct s2n_test_io_pair io_pair;
    struct s2n_config *config;
...
int main(int argc, char **argv)
{
    ...
    EXPECT_SUCCESS(run_test_config_swap_ch_cb(S2N_CLIENT_HELLO_CB_NONBLOCKING, chain_and_key, &client_hello_ctx));
    ...
    client_hello_ctx.swap_config_during_callback = 1;
    EXPECT_SUCCESS(run_test_config_swap_ch_cb(S2N_CLIENT_HELLO_CB_BLOCKING, chain_and_key, &client_hello_ctx));
    ...
    EXPECT_SUCCESS(run_test_no_config_swap_ch_cb(S2N_CLIENT_HELLO_CB_BLOCKING, chain_and_key, &client_hello_ctx));

    EXPECT_SUCCESS(run_test_no_config_swap_ch_cb(S2N_CLIENT_HELLO_CB_NONBLOCKING, chain_and_key, &client_hello_ctx));

    /** Test rejecting connection in client hello callback **/
    EXPECT_SUCCESS(run_test_reject_handshake_ch_cb(S2N_CLIENT_HELLO_CB_BLOCKING, chain_and_key, &client_hello_ctx));

    EXPECT_SUCCESS(run_test_reject_handshake_ch_cb(S2N_CLIENT_HELLO_CB_NONBLOCKING, chain_and_key, &client_hello_ctx));

    EXPECT_SUCCESS(run_test_poll_ch_cb(S2N_CLIENT_HELLO_CB_NONBLOCKING, chain_and_key, &client_hello_ctx));
