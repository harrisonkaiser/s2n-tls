...
static int s2n_test_read_handler(struct s2n_connection *conn)
{
    ...
    EXPECT_BYTEARRAY_EQUAL(s2n_stuffer_raw_read(&conn->handshake.io, TEST_DATA_SIZE), TEST_DATA, TEST_DATA_SIZE);
    return S2N_SUCCESS;
}
...
/* Writes handshake message */
{
    ...
    EXPECT_BYTEARRAY_EQUAL(s2n_stuffer_raw_read(&conn->out, sizeof(message_data)), message_data, sizeof(message_data));
...
/* Reads basic handshake message */
{
    ...
    EXPECT_BYTEARRAY_EQUAL(s2n_stuffer_raw_read(&conn->in, TEST_DATA_SIZE), TEST_DATA, sizeof(TEST_DATA));
...
/* Blocks on insufficient data for handshake message header */
{
    ...
    uint8_t actual_message_type = 0;
    EXPECT_ERROR_WITH_ERRNO(s2n_quic_read_handshake_message(conn, &actual_message_type), S2N_ERR_IO_BLOCKED);
...
/* Fails for an impossibly large handshake message */
{
    ...
    uint8_t actual_message_type = 0;
    EXPECT_ERROR_WITH_ERRNO(s2n_quic_read_handshake_message(conn, &actual_message_type), S2N_ERR_BAD_MESSAGE);
...
/* Functional Tests */
{
    ...
    for (size_t i = 1; i < server_hello.size - 1; i++) {
        ...
        EXPECT_SUCCESS(s2n_stuffer_write_bytes(&input_stuffer, server_hello.data + i, server_hello.size - i));
        EXPECT_FAILURE_WITH_ERRNO(s2n_negotiate(conn, &blocked_status), S2N_ERR_IO_BLOCKED);
    ...
    /* Functional: successfully writes full handshake message */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(s2n_stuffer_raw_read(&output_stuffer, TEST_DATA_SIZE), TEST_DATA, TEST_DATA_SIZE);
