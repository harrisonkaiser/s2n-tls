...
int main(int argc, char **argv)
{
    ...
    /* Test s2n_hex_string_to_bytes */
    {
        ...
        /* Test with output buffer too small */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(s2n_hex_string_to_bytes(long_input_str, &output_blob), S2N_ERR_INVALID_HEX);
        }
        ...
        /* Test with invalid characters */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_hex_string_to_bytes((const uint8_t *) "#2", &output_blob), S2N_ERR_INVALID_HEX);
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_hex_string_to_bytes((const uint8_t *) "1#", &output_blob), S2N_ERR_INVALID_HEX);
        }
        ...
        for (size_t i = 0; i < s2n_array_len(test_cases); i++) {
            ...
            EXPECT_BYTEARRAY_EQUAL(
                actual_output.data, test_cases[i].expected_output, test_cases[i].expected_output_size);
        }
    }
