...
/* s2n_tls13_serialize_resumption_state */
{
    ...
    /* Test TLS1.3 serialization with early data */
    {
        ...
        EXPECT_SUCCESS(s2n_connection_set_server_early_data_context(
            conn, test_early_data_context, sizeof(test_early_data_context)));
...
/* s2n_deserialize_resumption_state */
{
    ...
    /* Server processes TLS1.2 ticket with EMS data correctly */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_deserialize_resumption_state(conn, NULL, &output), S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);
        EXPECT_TRUE(conn->ems_negotiated);
    ...
    /* Deserialized ticket sets correct PSK values for early data */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(
            psk->early_data_config.application_protocol.data, expected_app_protocol, APP_PROTOCOL_LEN);
         ...
         * to worry about versioning. */
    {
        ...
        EXPECT_SUCCESS(s2n_stuffer_write_bytes(
            &ticket_stuffer, tls13_ticket_with_early_data, sizeof(tls13_ticket_with_early_data)));
    ...
    /* Fails if external PSKs already set */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_deserialize_resumption_state(conn, &conn->client_ticket, &ticket_stuffer), S2N_ERR_PSK_MODE);
    ...
    /* Functional test: The TLS1.3 client can deserialize what it serializes with early data */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(
            psk->early_data_config.application_protocol.data, test_app_protocol, test_app_protocol_len);
        EXPECT_EQUAL(psk->early_data_config.context.size, sizeof(test_early_data_context));
        EXPECT_BYTEARRAY_EQUAL(
            psk->early_data_config.context.data, test_early_data_context, sizeof(test_early_data_context));
...
/* s2n_encrypt_session_ticket */
{
    ...
    S2N_BLOB_FROM_HEX(ticket_key, "077709362c2e32df0ddc3f0dc47bba6390b6c73bb50f9c3122ec844ad7c2b3e5");
...
/* s2n_config_set_session_ticket_cb */
{
    ...
    /* Safety check */
    {
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_config_set_session_ticket_cb(NULL, s2n_test_session_ticket_callback, ctx), S2N_ERR_NULL);
    }
...
/* Ticket data is larger than customer buffer */
{
    ...
    size_t max_data_len = sizeof(data);
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_session_ticket_get_data(&session_ticket, max_data_len, data), S2N_ERR_SERIALIZED_SESSION_STATE_TOO_LONG);
}
}
