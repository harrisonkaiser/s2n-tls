...
static int s2n_x509_validator_validate_cert_chain_test(struct s2n_connection *conn, struct s2n_stuffer *stuffer)
{
    ...
    POSIX_GUARD_RESULT(s2n_x509_validator_validate_cert_chain(
        &conn->x509_validator, conn, cert_chain_data, cert_chain_size, &actual_cert_pkey_type, &public_key));
...
static int s2n_setup_connection_for_ocsp_validate_test(
    struct s2n_connection **conn, struct s2n_cert_chain_and_key *chain_and_key)
{
...
int main(int argc, char **argv)
{
    ...
    /* TLS1.2 does NOT send extensions */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(s2n_extension_list_parse(&stuffer, &extensions), S2N_ERR_BAD_MESSAGE);
    }
