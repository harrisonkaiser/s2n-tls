...
static S2N_RESULT s2n_compare_utf8_strings(
    struct s2n_blob *input_der, const char *expected_utf8_str, uint32_t utf8_len_in, uint32_t expected_utf8_len)
{
    ...
    RESULT_GUARD_POSIX(
        s2n_cert_get_utf8_string_from_extension_data(input_der->data, input_der->size, utf8_str.data, &utf8_str.size));
...
int main(int argc, char **argv)
{
    ...
    {
        DEFER_CLEANUP(
            struct s2n_cert_chain_and_key *chain = s2n_cert_chain_and_key_new(), s2n_cert_chain_and_key_ptr_free);
    ...
    /* Test s2n_connection_get_peer_cert_chain */
    {
        ...
        EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(
            &s2n_chain_and_key, S2N_DEFAULT_ECDSA_TEST_CERT_CHAIN, S2N_DEFAULT_ECDSA_TEST_PRIVATE_KEY));
        ...
        EXPECT_SUCCESS(
            s2n_config_set_verify_host_callback(config_skip_x509_verification, verify_host_fn, &verify_data));
        EXPECT_SUCCESS(s2n_config_set_verification_ca_location(
            config_skip_x509_verification, S2N_DEFAULT_ECDSA_TEST_CERT_CHAIN, NULL));
        ...
        EXPECT_SUCCESS(
            s2n_config_set_verify_host_callback(config_with_x509_verification, verify_host_fn, &verify_data));
        EXPECT_SUCCESS(s2n_config_set_verification_ca_location(
            config_with_x509_verification, S2N_DEFAULT_ECDSA_TEST_CERT_CHAIN, NULL));
        ...
        /* Test s2n_connection_get_peer_cert_chain failure cases with error codes */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_peer_cert_chain(server_conn, chain_and_key), S2N_ERR_INVALID_ARGUMENT);

            /* x509 verification is skipped on client side */
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_peer_cert_chain(client_conn, test_peer_chain), S2N_ERR_CERT_NOT_VALIDATED);
        ...
        if (s2n_x509_ocsp_stapling_supported()) {
            EXPECT_SUCCESS(
                s2n_cert_chain_and_key_set_ocsp_data(s2n_chain_and_key, ocsp_data, s2n_array_len(ocsp_data)));
    ...
    /* Test X509 Extension helper functions */
    {
        ...
        EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(&custom_cert_chain,
            S2N_RSA_2048_SHA256_INTERMEDIATE_CERT_CUSTOM_OID, S2N_RSA_2048_SHA256_INTERMEDIATE_CA_KEY));
        struct s2n_cert *cert = custom_cert_chain->cert_chain->head;
        EXPECT_NOT_NULL(cert);

        S2N_BLOB_FROM_HEX(subject_key_id_blob,
            "04 14 F9 19 58 9D 9E 97 89 9C 27 67 5B 62 19 \
                                                2A 1E 27 D6 4E 1E F6");
        S2N_BLOB_FROM_HEX(authority_key_id_blob,
            "30 16 80 14 56 9E 26 B6 09 4C 2E AC C8 4E 51 \
        ...
        S2N_BLOB_FROM_HEX(custom_oid_1_blob,
            "0C 41 6B 65 79 69 64 3A 33 \
        ...
        S2N_BLOB_FROM_HEX(custom_oid_2_blob,
            "0C 18 49 50 20 41 64 64 72 65 73 73 3A 31 32 2E \
                                              33 34 35 2E 36 37 2E 38 39 30");
        S2N_BLOB_FROM_HEX(custom_oid_3_blob,
            "0C 28 44 4E 53 3A 31 32 2E 33 34 35 2E 36 37 2E \
        ...
        struct {
            ...
            { .oid = "X509v3 Subject Key Identifier", .expected_der = subject_key_id_blob, .critical = false },
            { .oid = "X509v3 Authority Key Identifier", .expected_der = authority_key_id_blob, .critical = false },
            { .oid = "X509v3 Basic Constraints", .expected_der = basic_constraints_blob, .critical = true },
            { .oid = "X509v3 Key Usage", .expected_der = key_usage_blob, .critical = true },
            ...
            { .oid = "1.2.3.4.5.6.7890.1.2.100.3",
                ...
                .critical = false },
            { .oid = "1.2.3.4.5.6.7890.1.2.100", .critical = false },
        };
        ...
        /* Test s2n_cert_get_x509_extension_value_length */
        {
            /* Safety checks */
            {
                ...
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value_length(NULL, oid, &ext_value_len), S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value_length(cert, NULL, &ext_value_len), S2N_ERR_NULL);
            ...
            for (i = 0; i < s2n_array_len(test_cases) - 1; i++) {
                EXPECT_SUCCESS(s2n_cert_get_x509_extension_value_length(
                    cert, (const uint8_t *) test_cases[i].oid, &test_cases[i].ext_value_len));
        ...
        /* Test s2n_cert_get_x509_extension_value */
        {
            /* Safety checks */
            {
                ...
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(NULL, oid, ext_value.data, &ext_value.size, &critical),
                    S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, NULL, ext_value.data, &ext_value.size, &critical),
                    S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, oid, NULL, &ext_value.size, &critical), S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, oid, ext_value.data, NULL, &critical), S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, oid, ext_value.data, &ext_value.size, NULL), S2N_ERR_NULL);
            ...
            for (i = 0; i < s2n_array_len(test_cases) - 1; i++) {
                ...
                EXPECT_SUCCESS(s2n_cert_get_x509_extension_value(cert, (const uint8_t *) test_cases[i].oid,
                    test_cases[i].returned_der.data, &test_cases[i].returned_der.size, &critical));
                EXPECT_BYTEARRAY_EQUAL(
                    test_cases[i].returned_der.data, test_cases[i].expected_der.data, test_cases[i].expected_der.size);
            ...
            /* Test failure case for insufficient amount of memory allocated */
            {
                ...
                EXPECT_SUCCESS(s2n_alloc(
                    &ext_value, test_cases[insuf_test_case].returned_der.size - OFFSET_INSUFFICIENT_MEM_SIZE));
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, (const uint8_t *) test_cases[insuf_test_case].oid,
            ...
            /* Test failure case for invalid X509 extension OID */
            {
                ...
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_x509_extension_value(cert, (const uint8_t *) test_cases[invalid_test_case].oid,
        ...
        /* Test s2n_cert_get_utf8_string_from_extension_data_length */
        {
            /* Safety checks */
            {
                ...
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data_length(NULL, der_ext_value_len, &utf8_len),
                    S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data_length(der_ext_value, 0, &utf8_len), S2N_ERR_SAFETY);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data_length(der_ext_value, der_ext_value_len, 0),
                    S2N_ERR_NULL);
            }
            ...
            for (i = 0; i < s2n_array_len(test_cases) - 1; i++) {
                if (i > 3) {
                    EXPECT_SUCCESS(s2n_cert_get_utf8_string_from_extension_data_length(
                        test_cases[i].returned_der.data, test_cases[i].returned_der.size, &test_cases[i].utf8_len));
            ...
            /* Test failure case for insufficient amount of memory allocated */
            {
                ...
                EXPECT_SUCCESS(
                    s2n_alloc(&utf8_str, test_cases[insuf_test_case].returned_der.size - OFFSET_INSUFFICIENT_MEM_SIZE));
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data(test_cases[insuf_test_case].returned_der.data,
                        test_cases[insuf_test_case].returned_der.size, utf8_str.data, &utf8_str.size),
                    S2N_ERR_INSUFFICIENT_MEM_SIZE);
        ...
        /* Test s2n_cert_get_utf8_string_from_extension_data */
        {
            /* Safety checks */
            {
                ...
                EXPECT_FAILURE_WITH_ERRNO(s2n_cert_get_utf8_string_from_extension_data(
                                              NULL, der_ext_value_len, utf8_str.data, &utf8_str.size),
                    S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data(der_ext_value, 0, utf8_str.data, &utf8_str.size),
                    S2N_ERR_SAFETY);
                EXPECT_FAILURE_WITH_ERRNO(s2n_cert_get_utf8_string_from_extension_data(
                                              der_ext_value, der_ext_value_len, NULL, &utf8_str.size),
                    S2N_ERR_NULL);
                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_cert_get_utf8_string_from_extension_data(der_ext_value, der_ext_value_len, utf8_str.data, NULL),
                    S2N_ERR_NULL);
            ...
            for (i = 0; i < s2n_array_len(test_cases) - 1; i++) {
                if (i > 3) {
                    ...
                    EXPECT_ERROR_WITH_ERRNO(s2n_compare_utf8_strings(&test_cases[i].returned_der,
                                                (const char *) test_cases[i].expected_der.data, ext_value_MAX_LEN,
    ...
    /* Test s2n_connection_get_client_cert_chain */
    {
        ...
        DEFER_CLEANUP(
            struct s2n_connection *tls12_client_conn = s2n_connection_new(S2N_CLIENT), s2n_connection_ptr_free);
        DEFER_CLEANUP(
            struct s2n_connection *tls12_server_conn = s2n_connection_new(S2N_SERVER), s2n_connection_ptr_free);
        DEFER_CLEANUP(
            struct s2n_connection *tls13_client_conn = s2n_connection_new(S2N_CLIENT), s2n_connection_ptr_free);
        DEFER_CLEANUP(
            struct s2n_connection *tls13_server_conn = s2n_connection_new(S2N_SERVER), s2n_connection_ptr_free);
         ...
         */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_client_cert_chain(tls12_server_conn, &output, &output_len), S2N_ERR_NULL);
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_client_cert_chain(tls13_server_conn, &output, &output_len), S2N_ERR_NULL);
            EXPECT_NULL(output);
        ...
        /* Should error if called by client */
        {
            ...
            uint32_t output_len = 0;
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_client_cert_chain(tls12_client_conn, &output, &output_len), S2N_ERR_NULL);
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_client_cert_chain(tls13_client_conn, &output, &output_len), S2N_ERR_NULL);
            EXPECT_NULL(output);
            EXPECT_EQUAL(output_len, 0);
        }
    ...
    /* Test s2n_cert_chain_and_key_set_ocsp_data */
    {
        ...
        EXPECT_SUCCESS(
            s2n_test_cert_chain_and_key_new(&chain, S2N_DEFAULT_TEST_CERT_CHAIN, S2N_DEFAULT_TEST_PRIVATE_KEY));
