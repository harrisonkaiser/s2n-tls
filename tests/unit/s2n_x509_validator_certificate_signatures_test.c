...
int main(int argc, char **argv)
{
    ...
    /* s2n_is_certificate_sig_scheme_supported() */
    {
        ...
        {
            EXPECT_SUCCESS(
                s2n_read_test_pem(S2N_ECDSA_P256_PKCS1_CERT_CHAIN, (char *) cert_file, S2N_MAX_TEST_PEM_SIZE));
        ...
        {
            EXPECT_SUCCESS(
                s2n_read_test_pem(S2N_RSA_PSS_2048_SHA256_LEAF_CERT, (char *) cert_file, S2N_MAX_TEST_PEM_SIZE));
            certLen = strlen((const char *) cert_file);
            ...
            EXPECT_ERROR_WITH_ERRNO(
                s2n_validate_sig_scheme_supported(conn, cert, &test_certificate_signature_preferences),
    ...
    /* s2n_validate_certificate_signature */
    {
        ...
        /* Connection using the default_tls13 security policy ignores a SHA-1 signature on a root certificate */
        {
            ...
            EXPECT_SUCCESS(
                s2n_read_test_pem(S2N_SHA1_ROOT_SIGNATURE_CA_CERT, (char *) cert_file, S2N_MAX_TEST_PEM_SIZE));
            certLen = strlen((const char *) cert_file);
