...
int main(int argc, char **argv)
{
    ...
    /* Test s2n_server_key_share_extension.recv with supported curves */
    {
        ...
        do {
            ...
            EXPECT_EQUAL(server_send_conn->kex_params.server_ecc_evp_params.negotiated_curve->iana_id,
                client_recv_conn->kex_params.server_ecc_evp_params.negotiated_curve->iana_id);
    ...
    /* Test s2n_server_key_share_extension.recv with various sample payloads */
    {
        ...
        if (s2n_is_evp_apis_supported()) {
            ...
            const char *key_share_payloads[] = { /* x25519 */
                "001d00206b24ffd795c496899cd14b7742a5ffbdc453c23085a7f82f0ed1e0296adb9e0e",
                /* p256 */
                "001700410474cfd75c0ab7b57247761a277e1c92b5810dacb251bb758f43e9d15aaf292c4a2be43e886425ba55653ebb7a4f32"
                "fe368bacce3df00c618645cf1eb646f22552",
                /* p384 */
                "00180061040a27264201368540483e97d324a3093e11a5862b0a1be0cf5d8510bc47ec285f5304e9ec3ba01a0c375c3b6fa4bd"
                "0ad44aae041bb776aebc7ee92462ad481fe86f8b6e3858d5c41d0f83b0404f711832a4119aec3da2eac86266f424b50aa212"
            };
             ...
             * if tls1.3 not enabled */
            {
                ...
                EXPECT_SUCCESS(
                    s2n_connection_allow_response_extension(client_conn, s2n_server_key_share_extension.iana_value));
        ...
        /* Test error handling parsing broken/trancated p256 key share */
        {
            ...
            const char *p256 =
                "001700410474cfd75c0ab7b57247761a277e1c92b5810dacb251bb758f43e9d15aaf292c4a2be43e886425ba55653ebb7a4f32"
                "fe368bacce3df00c618645cf1eb6";

            EXPECT_NULL(client_conn->kex_params.server_ecc_evp_params.negotiated_curve);
            EXPECT_SUCCESS(s2n_stuffer_alloc_ro_from_hex_string(&extension_stuffer, p256));

            EXPECT_FAILURE_WITH_ERRNO(
                s2n_server_key_share_extension.recv(client_conn, &extension_stuffer), S2N_ERR_BAD_KEY_SHARE);
    ...
    /* Test Shared Key Generation */
    {
        ...
        EXPECT_EQUAL(server_conn->kex_params.server_ecc_evp_params.negotiated_curve,
            client_conn->kex_params.server_ecc_evp_params.negotiated_curve);

        /* Ensure both client and server public key matches */
        s2n_public_ecc_keys_are_equal(
            &server_conn->kex_params.server_ecc_evp_params, &client_conn->kex_params.server_ecc_evp_params);
        s2n_public_ecc_keys_are_equal(
            &server_conn->kex_params.client_ecc_evp_params, &client_conn->kex_params.client_ecc_evp_params);

        /* Server generates shared key based on Server's Key and Client's public key  */
        struct s2n_blob server_shared_secret = { 0 };
        EXPECT_SUCCESS(s2n_ecc_evp_compute_shared_secret_from_params(&server_conn->kex_params.server_ecc_evp_params,
            &server_conn->kex_params.client_ecc_evp_params, &server_shared_secret));

        /* Clients generates shared key based on Client's Key and Server's public key */
        struct s2n_blob client_shared_secret = { 0 };
        EXPECT_SUCCESS(s2n_ecc_evp_compute_shared_secret_from_params(&client_conn->kex_params.client_ecc_evp_params,
            &client_conn->kex_params.server_ecc_evp_params, &client_shared_secret));

        /* Test that server shared secret matches client shared secret */
        if (ecc_pref->ecc_curves[0] == &s2n_ecc_curve_secp256r1
            || ecc_pref->ecc_curves[0] == &s2n_ecc_curve_secp384r1) {
    ...
    if (s2n_is_evp_apis_supported()) {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_server_key_share_extension.recv(conn, extension_stuffer), S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
    ...
    /* Test the s2n_server_key_share_extension.recv with HelloRetryRequest */
    {
         ...
         * Test that s2n_server_key_share_extension.recv obtains the server negotiate curve successfully. */
        {
            ...
            EXPECT_MEMCPY_SUCCESS(
                server_conn->handshake_params.server_random, hello_retry_req_random, S2N_TLS_RANDOM_DATA_LEN);
            EXPECT_SUCCESS(s2n_server_key_share_extension.send(server_conn, key_share_extension));
            ...
            EXPECT_MEMCPY_SUCCESS(
                client_conn->handshake_params.server_random, hello_retry_req_random, S2N_TLS_RANDOM_DATA_LEN);
...

    {
        ...
        /* Tests for s2n_server_key_share_extension.recv with hybrid PQ key shares */
        {
            ...
            if (!s2n_pq_is_enabled()) {
                ...
                EXPECT_SUCCESS(s2n_stuffer_write_uint16(
                    &iana_stuffer, test_security_policy.kem_preferences->tls13_kem_groups[0]->iana_id));

                EXPECT_FAILURE_WITH_ERRNO(
                    s2n_server_key_share_extension.recv(client_conn, &iana_stuffer), S2N_ERR_PQ_DISABLED);
            ...
            if (s2n_pq_is_enabled()) {
                {
                    for (size_t i = 0; i < s2n_array_len(test_kem_groups); i++) {
                        ...
                        struct s2n_blob *pq_private_key =
                        ...
                        EXPECT_SUCCESS(s2n_read_server_key_share_hybrid_test_vectors(
                            kem_group, pq_private_key, &pq_shared_secret, &key_share_payload));
                        ...
                        EXPECT_SUCCESS(s2n_ecc_evp_generate_ephemeral_key(
                            &client_conn->kex_params.client_kem_group_params.ecc_params));
                        ...
                        EXPECT_EQUAL(client_conn->kex_params.server_kem_group_params.ecc_params.negotiated_curve,
                            kem_group->curve);
                        ...
                        EXPECT_EQUAL(client_conn->kex_params.client_kem_group_params.ecc_params.negotiated_curve,
                            kem_group->curve);
                        ...
                        EXPECT_BYTEARRAY_EQUAL(
                            client_conn->kex_params.client_kem_group_params.kem_params.shared_secret.data,
                ...
                /* Various failure cases */
                {
                    for (size_t i = 0; i < s2n_array_len(test_kem_groups); i++) {
                        ...
                        const char *bad_group =
                            "2F2C"; /* IANA ID for secp256r1_threebears-babybear-r2 (not imported into s2n) */
                        ...
                        EXPECT_FAILURE_WITH_ERRNO(
                            s2n_server_key_share_extension.recv(client_conn, &wrong_share_stuffer),
                            S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
                        ...
                        size_t pq_share_size_index = S2N_SIZE_OF_NAMED_GROUP
                        ...
                        key_share_payload.blob.data[pq_share_size_index] =
                            ~key_share_payload.blob.data[pq_share_size_index];
                        EXPECT_FAILURE_WITH_ERRNO(s2n_server_key_share_extension.recv(client_conn, &key_share_payload),
                            S2N_ERR_BAD_KEY_SHARE);
                        /* Revert key_share_payload back to correct state */
                        ...
                        key_share_payload.blob.data[pq_key_share_first_byte_index] =
                            ~key_share_payload.blob.data[pq_key_share_first_byte_index];
                        EXPECT_SUCCESS(s2n_server_key_share_extension.recv(client_conn, &key_share_payload));
                        EXPECT_BYTEARRAY_NOT_EQUAL(
                            client_conn->kex_params.client_kem_group_params.kem_params.shared_secret.data,
                            pq_shared_secret.blob.data, kem_group->kem->shared_secret_key_length);
        ...
        /* Test s2n_server_key_share_send_check_pq_hybrid */
        {
            ...
            if (s2n_pq_is_enabled()) {
                ...
                EXPECT_SUCCESS(
                    s2n_ecc_evp_generate_ephemeral_key(&conn->kex_params.client_kem_group_params.ecc_params));
                EXPECT_FAILURE_WITH_ERRNO(s2n_server_key_share_send_check_pq_hybrid(conn), S2N_ERR_BAD_KEY_SHARE);
        ...
        if (s2n_pq_is_enabled()) {
            for (size_t i = 0; i < S2N_SUPPORTED_KEM_GROUPS_COUNT; i++) {
                ...
                EXPECT_EQUAL(
                    s2n_stuffer_data_available(&stuffer), (2 * sizeof(uint16_t)) + kem_group->server_share_size);
        ...
        /* Test s2n_server_key_share_extension.send fails when both server_kem_group and server_curve are non-NULL */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_server_key_share_extension.send(conn, &stuffer), S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
    ...
    struct s2n_blob *pq_private_key, struct s2n_stuffer *pq_shared_secret, struct s2n_stuffer *key_share_payload)
{
    ...
    POSIX_GUARD(
        ReadHex(kat_file, pq_shared_secret_ptr, kem_group->kem->shared_secret_key_length, "pq_shared_secret = "));

    size_t key_share_payload_size = S2N_SIZE_OF_NAMED_GROUP
        + S2N_SIZE_OF_KEY_SHARE_SIZE /* Not a typo; PQ shares have an overall (combined) size */
        + S2N_SIZE_OF_KEY_SHARE_SIZE /* and a size for each contribution of the hybrid share. */
        + kem_group->curve->share_size + S2N_SIZE_OF_KEY_SHARE_SIZE + kem_group->kem->ciphertext_length;
