...
static S2N_RESULT s2n_test_client_and_server_new(
...
static S2N_RESULT s2n_test_set_blocking_stuffer_io(struct s2n_connection *client_conn,
    struct s2n_connection *server_conn, struct s2n_stuffer *client_in, struct s2n_stuffer *server_in)
{
...
int main(int argc, char **argv)
{
    ...
    uint8_t malformed_record[] = { TLS_HANDSHAKE, 0x03, 0x03, 0x00, 0x04, TLS_FINISHED, 0x00, 0x00, 0x00 };
...
/* Propagate errors from s2n_negotiate */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_send_early_data(client_conn, test_data, sizeof(test_data), &data_size, &blocked), S2N_ERR_BAD_MESSAGE);
...
/* Propagate errors from s2n_send */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_send_early_data(client_conn, test_data, sizeof(test_data), &data_size, &blocked), S2N_ERR_REENTRANCY);
    EXPECT_EQUAL(s2n_conn_get_current_message_type(client_conn), SERVER_HELLO);
    EXPECT_EQUAL(data_size, 0);
...
/* Propagate errors from s2n_negotiate */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_recv_early_data(server_conn, payload, sizeof(payload), &data_size, &blocked), S2N_ERR_BAD_MESSAGE);
...
/* Propagate errors from s2n_recv */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_recv_early_data(server_conn, payload, sizeof(payload), &data_size, &blocked), S2N_ERR_REENTRANCY);
...
/* Send early data once */
{
    ...
    EXPECT_BLOCKED_ON_IO(
        s2n_recv_early_data(server_conn, actual_payload, sizeof(actual_payload), &data_size, &blocked));
    EXPECT_BLOCKED_ON(server_conn, blocked, S2N_BLOCKED_ON_READ, END_OF_EARLY_DATA);
    EXPECT_EQUAL(data_size, sizeof(test_data));
...
/* Receive early data too large for buffer */
{
    ...
    EXPECT_SUCCESS(s2n_recv_early_data(server_conn, actual_payload, sizeof(actual_payload), &data_size, &blocked));
...
/* Send multiple early data messages */
{
    ...
    for (size_t i = 0; i < 10; i++) {
        EXPECT_SUCCESS(s2n_send_early_data(client_conn, test_data, sizeof(test_data), &data_size, &blocked));
     ...
     */
{
    ...
    while (true) {
        s2n_allowed_writes = 1;
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_send_early_data(client_conn, large_data, sizeof(large_data), &data_size, &blocked), S2N_ERR_IO_BLOCKED);
...
if (s2n_is_evp_apis_supported()) {
    ...
    S2N_BLOB_FROM_HEX(early_record,
    ...
    /* Test s2n_recv_early_data without any blocking */
    {
        ...
        EXPECT_SUCCESS(s2n_recv_early_data(server_conn, actual_payload, sizeof(actual_payload), &data_read, &blocked));
        EXPECT_NOT_BLOCKED(server_conn, blocked, CLIENT_FINISHED);
    ...
    /* Test s2n_recv_early_data with blocking */
    {
        ...
        for (size_t i = 0; i < repeat_count; i++) {
            EXPECT_BLOCKED_ON_IO(
                s2n_recv_early_data(server_conn, actual_payload, sizeof(actual_payload), &data_read, &blocked));
            EXPECT_BLOCKED_ON(server_conn, blocked, S2N_BLOCKED_ON_READ, CLIENT_HELLO);
