...
static int s2n_test_select_psk_identity_callback(
    struct s2n_connection *conn, void *context, struct s2n_offered_psk_list *psk_identity_list)
{
...
static int s2n_test_error_select_psk_identity_callback(
    struct s2n_connection *conn, void *context, struct s2n_offered_psk_list *psk_identity_list)
{
...
int main(int argc, char **argv)
{
    ...
    /* Test: s2n_generate_obfuscated_ticket_age */
    {
        ...
        struct {
            ...
            { .current_time = MILLIS_TO_NANOS(50),
                .ticket_issue_time = 0,
                .ticket_age_add = 50,
                .expected_output = 100 },
            { .current_time = MILLIS_TO_NANOS(500),
                .ticket_issue_time = 0,
                .ticket_age_add = 50,
                .expected_output = 550 },
            { .current_time = MILLIS_TO_NANOS(UINT32_MAX),
                .ticket_issue_time = 0,
                .ticket_age_add = 1,
                .expected_output = 0 },
            { .current_time = MILLIS_TO_NANOS(UINT32_MAX),
    ...
    /* Test: s2n_client_psk_send */
    {
        ...
        /* Send multiple PSK identities */
        {
            ...
            struct s2n_psk_test_case test_cases[] = {
                ...
                { .hmac_alg = S2N_HMAC_SHA384,
                    ...
                    .identity_size = sizeof(test_identity_2) },
            };
            ...
            for (size_t i = 0; i < s2n_array_len(test_cases); i++) {
                ...
                binder_list_size += test_cases[i].hash_size + sizeof(uint8_t) /* size of binder size */;
            }
        ...
        /* Send a resumption psk identity */
        {
            ...
            EXPECT_SUCCESS(s2n_stuffer_skip_read(&out,
                sizeof(uint16_t) /* identity_list_size */ + sizeof(uint16_t) /* identity_size */
                    + sizeof(test_identity)));
         ...
         */
        {
            ...
            const struct s2n_psk_test_case matching_psk = { .hmac_alg = S2N_HMAC_SHA384,
                .hash_size = SHA384_DIGEST_LENGTH,
                .identity = test_identity,
                .identity_size = sizeof(test_identity) };
            const struct s2n_psk_test_case non_matching_psk = { .hmac_alg = S2N_HMAC_SHA224,
                .hash_size = SHA224_DIGEST_LENGTH,
                .identity = test_identity,
                .identity_size = sizeof(test_identity) };
            ...
            EXPECT_SUCCESS(s2n_stuffer_read_uint16(&out, &identity_list_size));
            EXPECT_EQUAL(
                identity_list_size, sizeof(identity_size) + matching_psk.identity_size + sizeof(obfuscated_ticket_age));
            ...
            EXPECT_EQUAL(s2n_stuffer_data_available(&out),
                matching_psk.hash_size + sizeof(uint8_t) /* size of binder size */
                    + sizeof(uint16_t))
            /* size of binder list size */;
...
#define LOCAL_IDENTITIES(list) .local_identities = list, .local_identities_len = s2n_array_len(list)
            ...
            { .match = true,
                WIRE_IDENTITIES(only_psk1),
                LOCAL_IDENTITIES(only_psk1),
                .wire_match_index = 0,
                .local_match_index = 0 },
            ...
            { .match = true,
                WIRE_IDENTITIES(only_psk1),
                LOCAL_IDENTITIES(all_psks_list),
                .wire_match_index = 0,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(only_psk2),
                LOCAL_IDENTITIES(all_psks_list),
                .wire_match_index = 0,
                .local_match_index = 1 },
            { .match = true,
                WIRE_IDENTITIES(only_psk3),
                LOCAL_IDENTITIES(all_psks_list),
                .wire_match_index = 0,
                .local_match_index = 2 },
            ...
            { .match = true,
                WIRE_IDENTITIES(all_psks_list),
                LOCAL_IDENTITIES(only_psk1),
                .wire_match_index = 0,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(all_psks_list),
                LOCAL_IDENTITIES(only_psk2),
                .wire_match_index = 1,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(all_psks_list),
                LOCAL_IDENTITIES(only_psk3),
                .wire_match_index = 2,
                .local_match_index = 0 },
            ...
            { .match = true,
                ...
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(reverse_order_list),
                LOCAL_IDENTITIES(all_psks_list),
                .wire_match_index = 2,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(all_psks_list),
                LOCAL_IDENTITIES(reverse_order_list),
                .wire_match_index = 2,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(list_without_psk1),
                LOCAL_IDENTITIES(all_psks_list),
                .wire_match_index = 0,
                .local_match_index = 1 },
            { .match = true,
                WIRE_IDENTITIES(all_psks_list),
                LOCAL_IDENTITIES(list_without_psk1),
                .wire_match_index = 1,
                .local_match_index = 0 },
            { .match = true,
                WIRE_IDENTITIES(list_without_psk1),
            ...
            /* Handle duplicates */
            { .match = true,
                WIRE_IDENTITIES(multiple_psk1),
                LOCAL_IDENTITIES(multiple_psk1),
                .wire_match_index = 0,
                .local_match_index = 0 },
        ...
        for (size_t i = 0; i < s2n_array_len(test_cases); i++) {
            ...
            for (size_t wire_i = 0; wire_i < test_cases[i].wire_identities_len; wire_i++) {
                EXPECT_OK(
                    s2n_write_test_identity(&client_identity_list.wire_data, test_cases[i].wire_identities[wire_i]));
            }
...
/* Customer selection logic: customer rejects all identities */
{
    ...
    struct s2n_stuffer wire_identities_in = { 0 };
    EXPECT_ERROR_WITH_ERRNO(s2n_client_psk_recv_identity_list(conn, &wire_identities_in), S2N_ERR_UNIMPLEMENTED);
...
/* Customer selection logic: customer selects valid identity */
{
    ...
    EXPECT_SUCCESS(
        s2n_config_set_psk_selection_callback(config, s2n_test_select_psk_identity_callback, &expected_wire_choice));
...
/* PSK type is resumption */
{
    ...
    uint16_t expected_wire_choice = 0;
    EXPECT_SUCCESS(s2n_config_set_psk_selection_callback(
        config_with_cb, s2n_test_select_psk_identity_callback, &expected_wire_choice));
...
/* Test: s2n_client_psk_recv_binder_list */
{
    ...
    EXPECT_SUCCESS(s2n_blob_init(&partial_client_hello, partial_client_hello_data, sizeof(partial_client_hello_data)));
    ...
    /* Receive an empty list */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_client_psk_recv_binder_list(conn, &partial_client_hello, &empty_wire_binders_in), S2N_ERR_BAD_MESSAGE);
    ...
    /* No chosen identity */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_client_psk_recv_binder_list(conn, &partial_client_hello, &wire_binders_in), S2N_ERR_NULL);
    }
    ...
    /* Binder list too short for chosen identity index */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_client_psk_recv_binder_list(conn, &partial_client_hello, &wire_binders_in), S2N_ERR_BAD_MESSAGE);
    }
    ...
    /* Binder for chosen identity is zero-length */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_client_psk_recv_binder_list(conn, &partial_client_hello, &wire_binders_in), S2N_ERR_SAFETY);
    }
...
/* Test: s2n_client_psk_recv */
{
    ...
    /* Receive a valid extension */
    {
        ...
        EXPECT_SUCCESS(s2n_stuffer_write_bytes(client_out, client_hello_prefix_data, sizeof(client_hello_prefix_data)));
        ...
        EXPECT_SUCCESS(s2n_stuffer_copy(client_out, server_in, s2n_stuffer_data_available(client_out)));
