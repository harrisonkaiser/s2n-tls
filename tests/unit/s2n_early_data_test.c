    ...
    const s2n_early_state_sequence *valid_early_state_sequences, size_t valid_early_state_sequences_len)
{
    ...
    for (s2n_early_data_state next_state = 0; next_state < S2N_EARLY_DATA_STATES_COUNT; next_state++) {
        ...
        if (expected_valid) {
            RESULT_GUARD(s2n_test_all_early_data_sequences(
                conn, i + 1, valid_early_state_sequences, valid_early_state_sequences_len));
        }
    }
...
int main(int argc, char **argv)
{
    ...
    /* Test s2n_connection_set_early_data_state */
    {
        ...
        const s2n_early_data_state early_data_not_requested_seq[] = { S2N_UNKNOWN_EARLY_DATA_STATE,
            S2N_EARLY_DATA_NOT_REQUESTED };
        const s2n_early_data_state early_data_rejected_seq[] = { S2N_UNKNOWN_EARLY_DATA_STATE, S2N_EARLY_DATA_REQUESTED,
            S2N_EARLY_DATA_REJECTED };
        const s2n_early_data_state early_data_success_seq[] = { S2N_UNKNOWN_EARLY_DATA_STATE, S2N_EARLY_DATA_REQUESTED,
            S2N_EARLY_DATA_ACCEPTED, S2N_END_OF_EARLY_DATA };
        ...
        /* Test known valid / invalid transitions */
        {
            ...
            EXPECT_ERROR_WITH_ERRNO(
                s2n_connection_set_early_data_state(conn, S2N_END_OF_EARLY_DATA), S2N_ERR_INVALID_EARLY_DATA_STATE);
         ...
         * can only be used to iterate through the known valid sequences. */
        {
            /* Test with the correct expected sequences */
            {
                ...
                struct s2n_connection *conn = s2n_connection_new(S2N_SERVER);
                EXPECT_NOT_NULL(conn);
                EXPECT_OK(s2n_test_all_early_data_sequences(
                    conn, 0, valid_early_state_sequences, s2n_array_len(valid_early_state_sequences)));
                EXPECT_SUCCESS(s2n_connection_free(conn));
            }

            /* Sanity check: adding an invalid expected sequence causes test to fail */
            {
                const s2n_early_data_state invalid_seq[] = { S2N_UNKNOWN_EARLY_DATA_STATE, S2N_EARLY_DATA_ACCEPTED,
                ...
                struct s2n_connection *conn = s2n_connection_new(S2N_SERVER);
                EXPECT_NOT_NULL(conn);
                EXPECT_ERROR(s2n_test_all_early_data_sequences(
                    conn, 0, test_early_state_sequences, s2n_array_len(test_early_state_sequences)));
                EXPECT_SUCCESS(s2n_connection_free(conn));
            }
    ...
    /* Test s2n_psk_configure_early_data */
    {
        ...
        /* Set cipher suite must match hmac algorithm */
        {
            ...
            EXPECT_SUCCESS(s2n_psk_configure_early_data(
                psk, nonzero_max_early_data, cipher_suite->iana_value[0], cipher_suite->iana_value[1]));
    ...
    /* Test s2n_early_data_config_clone */
    {
        ...
        for (size_t called_directly = 0; called_directly < 2; called_directly++) {
            ...
            EXPECT_SUCCESS(s2n_psk_configure_early_data(
                original, test_max_early_data, test_cipher_suite->iana_value[0], test_cipher_suite->iana_value[1]));
        ...
        **/
        {
            ...
            EXPECT_OK(s2n_append_test_chosen_psk_with_early_data(
            ...
            **/
            {
                ...
                EXPECT_SUCCESS(s2n_psk_set_application_protocol(
                    conn->psk_params.chosen_psk, required_protocol, sizeof(required_protocol)));
    ...
    /* Test s2n_early_data_accept_or_reject */
    {
        ...
        /* Triggers callback to let application reject early data */
        {
            ...
            conn->early_data_state = S2N_EARLY_DATA_REQUESTED;
            EXPECT_SUCCESS(s2n_psk_set_early_data_context(
                conn->psk_params.chosen_psk, &accept_early_data, sizeof(accept_early_data)));
            EXPECT_OK(s2n_early_data_accept_or_reject(conn));
            EXPECT_EQUAL(conn->early_data_state, S2N_EARLY_DATA_ACCEPTED);
    ...
    /* Test s2n_connection_get_early_data_status */
    {
        ...
        /* Safety */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_early_data_status(&conn, &status), S2N_ERR_INVALID_EARLY_DATA_STATE);
        }
    ...
    /* Test s2n_connection_get_remaining_early_data_size */
    {
        ...
        /* If early data allowed, return the remaining bytes allowed */
        {
            ...
            conn->early_data_state = S2N_EARLY_DATA_REQUESTED;
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_connection_get_remaining_early_data_size(conn, &bytes), S2N_ERR_MAX_EARLY_DATA_SIZE);
            EXPECT_EQUAL(bytes, 0);
...
/* Test s2n_offered_early_data_get_context and s2n_offered_early_data_get_context_length */
{
    ...
    /* Context */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_offered_early_data_get_context(&early_data, actual_context, 1), S2N_ERR_INSUFFICIENT_MEM_SIZE);
