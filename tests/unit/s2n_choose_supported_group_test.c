...
/* Test for PQ */
{
         ...
         * should be chosen. */
    {
        ...
        EXPECT_EQUAL(server_conn->kex_params.server_kem_group_params.ecc_params.negotiated_curve,
            kem_pref->tls13_kem_groups[0]->curve);
        EXPECT_EQUAL(
            server_conn->kex_params.server_kem_group_params.kem_params.kem, kem_pref->tls13_kem_groups[0]->kem);
        EXPECT_NULL(server_conn->kex_params.server_ecc_evp_params.negotiated_curve);
        EXPECT_SUCCESS(s2n_connection_free(server_conn));
    }
/* Need at least two KEM's to test fallback */
         ...
         * group should be chosen. */
    {
        ...
        EXPECT_EQUAL(server_conn->kex_params.server_kem_group_params.ecc_params.negotiated_curve,
            kem_pref->tls13_kem_groups[1]->curve);
        EXPECT_EQUAL(
            server_conn->kex_params.server_kem_group_params.kem_params.kem, kem_pref->tls13_kem_groups[1]->kem);
        EXPECT_NULL(server_conn->kex_params.server_ecc_evp_params.negotiated_curve);
        EXPECT_SUCCESS(s2n_connection_free(server_conn));
    }
#endif
