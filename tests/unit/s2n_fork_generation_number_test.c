...
static int s2n_unit_test_thread(uint64_t expected_fork_generation_number)
{
    ...
    for (size_t thread_index = 0; thread_index < MAX_NUMBER_OF_TEST_THREADS; thread_index++) {
        EXPECT_EQUAL(pthread_create(&threads[thread_index], NULL, &s2n_unit_test_thread_get_fgn,
                         (void *) &expected_fork_generation_number),
            0);
    }
...
static int s2n_unit_test_clone(uint64_t parent_process_fgn)
{
    ...
    int proc_pid = clone(s2n_unit_test_clone_child_process, (void *) (process_child_stack + PROCESS_CHILD_STACK_SIZE),
        0, (void *) &return_fork_generation_number);
...
static int s2n_unit_tests_common(struct fgn_test_case *test_case)
{
    ...
    if (test_case->test_case_must_pass_clone_test == CLONE_TEST_YES) {
        EXPECT_EQUAL(
            (s2n_is_madv_wipeonfork_supported() == true) || (s2n_is_map_inherit_zero_supported() == true), true);
