...
int main(int argc, char **argv)
{
    ...
    EXPECT_SUCCESS(s2n_blob_init(&derived_secret, derived_secret_bytes, S2N_TLS13_SECRET_MAX_LEN));
     ...
     */
    {
        ...
        /* Extract EARLY_SECRET */
        {
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.extract_secret, early_secret.data, early_secret.size);
         ...
         */
        S2N_BLOB_FROM_HEX(handshake_secret,
...
#if EVP_APIS_SUPPORTED && S2N_OPENSSL_VERSION_AT_LEAST(1, 1, 1)
        {
             ...
             */
            S2N_BLOB_FROM_HEX(client_priv,
                "49 af 42 ba 7f 79 94 85 2d 71 3e f2 78 \
                   4b cb ca a7 91 1d e2 6a dc 56 42 cb 63 45 40 e7 ea 50 05");
            S2N_BLOB_FROM_HEX(client_pub,
             ...
             */
            S2N_BLOB_FROM_HEX(server_priv,
                "b1 58 0e ea df 6d d5 89 b8 ef 4f 2d 56 \
                      52 57 8c c8 10 e9 98 01 91 ec 8d 05 83 08 ce a2 16 a2 1e");
            S2N_BLOB_FROM_HEX(server_pub,
            ...
            /* Server */
            {
                ...
                conn->kex_params.client_ecc_evp_params.evp_pkey =
                    EVP_PKEY_new_raw_public_key(openssl_type, NULL, (unsigned char *) client_pub.data, client_pub.size);
                ...
                EXPECT_BYTEARRAY_EQUAL(
                    conn->secrets.tls13.extract_secret, handshake_secret.data, handshake_secret.size);
            ...
            /* Client */
            {
                ...
                conn->kex_params.server_ecc_evp_params.negotiated_curve = curve;
                conn->kex_params.server_ecc_evp_params.evp_pkey =
                    EVP_PKEY_new_raw_public_key(openssl_type, NULL, (unsigned char *) server_pub.data, server_pub.size);
                EXPECT_NOT_NULL(conn->kex_params.server_ecc_evp_params.evp_pkey);
        ...
        /* Derive S2N_CLIENT_HANDSHAKE_TRAFFIC_SECRET */
        {
             ...
             */
            S2N_BLOB_FROM_HEX(finished_key,
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_OK(s2n_tls13_derive_secret(conn, S2N_HANDSHAKE_SECRET, S2N_CLIENT, &derived_secret));

                EXPECT_EQUAL(derived_secret.size, secret.size);
                EXPECT_BYTEARRAY_EQUAL(derived_secret.data, secret.data, secret.size);
                EXPECT_BYTEARRAY_EQUAL(conn->handshake.client_finished, finished_key.data, finished_key.size);
            }
        }
        ...
        /* Derive S2N_SERVER_HANDSHAKE_TRAFFIC_SECRET */
        {
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_OK(s2n_tls13_derive_secret(conn, S2N_HANDSHAKE_SECRET, S2N_SERVER, &derived_secret));
                EXPECT_EQUAL(derived_secret.size, secret.size);
                EXPECT_BYTEARRAY_EQUAL(derived_secret.data, secret.data, secret.size);
                EXPECT_BYTEARRAY_EQUAL(conn->handshake.server_finished, finished_key.data, finished_key.size);
            }
        }
         ...
         */
        S2N_BLOB_FROM_HEX(master_secret,
         ...
         */
        {
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.extract_secret, master_secret.data, master_secret.size);
        ...
        /* Derive CLIENT_APPLICATION_TRAFFIC_SECRET_0 */
        {
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_OK(s2n_tls13_derive_secret(conn, S2N_MASTER_SECRET, S2N_CLIENT, &derived_secret));
                EXPECT_EQUAL(derived_secret.size, secret.size);
                EXPECT_BYTEARRAY_EQUAL(derived_secret.data, secret.data, secret.size);
            }
        }
        ...
        /* Derive SERVER_APPLICATION_TRAFFIC_SECRET_0 */
        {
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_OK(s2n_tls13_derive_secret(conn, S2N_MASTER_SECRET, S2N_SERVER, &derived_secret));
        ...
        /* Derive RESUMPTION_MASTER_SECRET */
        {
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_EQUAL(derived_secret.size, secret.size);
                EXPECT_BYTEARRAY_EQUAL(conn->secrets.tls13.resumption_master_secret, secret.data, secret.size);
            }
        }
    }
    ...
    /* Resumed 0-RTT Handshake */
    {
        ...
        /* Derive BINDER_KEY */
        {
             ...
             */
            S2N_BLOB_FROM_HEX(binder_key,
                "69 fe 13 1a 3b ba d5 d6 3c 64 ee bc c3 0e 39 5b \
                         9d 81 07 72 6a 13 d0 74 e3 89 db c8 a4 e4 72 56");
        ...
        /* Derive CLIENT_EARLY_TRAFFIC_SECRET */
        {
            ...
            S2N_BLOB_FROM_HEX(hash,
                "08 ad 0f a0 5d 7c 72 33 b1 77 5b a2 ff 9f 4c 5b \
                         8b 59 27 6b 7f 22 7f 13 a9 76 24 5f 5d 96 09 13");
            S2N_BLOB_FROM_HEX(secret,
                "3f bb e6 a6 0d eb 66 c3 0a 32 79 5a ba 0e \
                         ff 7e aa 10 10 55 86 e7 be 5c 09 67 8d 63 b6 ca ab 62");
            ...
            for (size_t i = 0; i < s2n_array_len(modes); i++) {
                ...
                EXPECT_OK(s2n_tls13_derive_secret(conn, S2N_EARLY_SECRET, S2N_CLIENT, &derived_secret));
                EXPECT_EQUAL(derived_secret.size, secret.size);
                EXPECT_BYTEARRAY_EQUAL(derived_secret.data, secret.data, secret.size);
            }
        }
