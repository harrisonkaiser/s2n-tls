...
#define TEST_SLACK 4
#define MAX_MEM_PER_CONNECTION ((MEM_PER_CONNECTION + TEST_SLACK) * 1024)

/* This is the total maximum memory allowed */
#define MAX_MEM_ALLOWED(num_connections) (2 * (num_connections) *MAX_MEM_PER_CONNECTION)
...
#define ACTUAL_MEM_PER_CONNECTION(num_connections, max_mem) ((((max_mem) / 2 / (num_connections)) / 1024) - TEST_SLACK)
...
int main(int argc, char **argv)
{
     ...
     */
#ifndef __APPLE__
    if (allocation_diff > maxAllowedMemDiff || handshake_diff > maxAllowedMemDiff
