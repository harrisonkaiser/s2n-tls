...
static int s2n_test_noop_secret_handler(
...
int main(int argc, char **argv)
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_connection_set_quic_transport_parameters(NULL, TEST_DATA, sizeof(TEST_DATA)), S2N_ERR_NULL);
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_connection_set_quic_transport_parameters(&conn, NULL, sizeof(TEST_DATA)), S2N_ERR_NULL);
...
EXPECT_FAILURE_WITH_ERRNO(
    s2n_connection_get_quic_transport_parameters(NULL, &data_buffer, &data_buffer_len), S2N_ERR_NULL);
EXPECT_FAILURE_WITH_ERRNO(s2n_connection_get_quic_transport_parameters(&conn, NULL, &data_buffer_len), S2N_ERR_NULL);
EXPECT_FAILURE_WITH_ERRNO(s2n_connection_get_quic_transport_parameters(&conn, &data_buffer, NULL), S2N_ERR_NULL);
}
...
/* Test s2n_connection_set_secret_callback */
{
    ...
    /* Safety checks */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_connection_set_secret_callback(NULL, s2n_test_noop_secret_handler, &test_context), S2N_ERR_NULL);
...
/* Test: no API that sends/receives application data is allowed when QUIC is enabled */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_sendv_with_offset(conn, &iovec_buffer, 1, 0, &blocked_status), S2N_ERR_UNSUPPORTED_WITH_QUIC);
