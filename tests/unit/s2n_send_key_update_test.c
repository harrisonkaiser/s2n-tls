...
int main(int argc, char **argv)
{
    ...
    /* s2n_send sends key update if necessary */
    {
        ...
        EXPECT_BYTEARRAY_NOT_EQUAL(server_conn->secrets.tls13.server_app_secret,
            client_conn->secrets.tls13.server_app_secret, S2N_TLS13_SECRET_MAX_LEN);
        EXPECT_BYTEARRAY_EQUAL(
            server_conn->secure->server_sequence_number, zero_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
        ...
        EXPECT_BYTEARRAY_EQUAL(client_conn->secrets.tls13.server_app_secret,
            server_conn->secrets.tls13.server_app_secret, S2N_TLS13_SECRET_MAX_LEN);
        EXPECT_BYTEARRAY_EQUAL(
            client_conn->secure->server_sequence_number, zero_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
    ...
    /* TLS 1.2 Server that receives TLS 1.3 KeyUpdate from Client should close connection */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_recv(server_conn, server_message, sizeof(server_message), &blocked), S2N_ERR_BAD_MESSAGE);
