...
int main(int argc, char **argv)
{
    ...
    /* Test s2n_client_early_data_indication_should_send */
    {
        ...
        /* Don't send if cipher suite not allowed by cipher preferences */
        {
            ...
            EXPECT_OK(
                s2n_append_test_psk_with_early_data(conn, nonzero_max_early_data, &s2n_rsa_with_3des_ede_cbc_sha));
            EXPECT_FALSE(s2n_client_early_data_indication_extension.should_send(conn));
        ...
        /* Don't send if application layer protocol not allowed by preferences */
        {
            ...
            EXPECT_SUCCESS(s2n_connection_append_protocol_preference(
                conn, other_app_protocol_data, sizeof(other_app_protocol_data)));
            EXPECT_SUCCESS(s2n_connection_append_protocol_preference(
                conn, other_app_protocol_data, sizeof(other_app_protocol_data)));
...
/* Test s2n_client_early_data_indiction_recv */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_client_early_data_indication_extension.recv(conn, NULL), S2N_ERR_UNSUPPORTED_EXTENSION);
...
EXPECT_SUCCESS(s2n_stuffer_copy(
    &client_conn->handshake.io, &server_conn->handshake.io, s2n_stuffer_data_available(&client_conn->handshake.io)));
