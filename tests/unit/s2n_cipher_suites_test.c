    ...
    for (size_t policy_index = 0; security_policy_selection[policy_index].version != NULL; policy_index++) {
        ...
        for (size_t cipher_index = 0; cipher_index < cipher_preferences->count; cipher_index++) {
            ...
            for (size_t all_index = 0; all_index < cipher_preferences_test_all.count; all_index++) {
                if (0
                    == memcmp(cipher_preferences->suites[cipher_index]->iana_value,
                        cipher_preferences_test_all.suites[all_index]->iana_value, S2N_TLS_CIPHER_SUITE_LEN)) {
                    EXPECT_NULL(match);
...
/* Test s2n_cipher_suite_from_iana */
{
    ...
    /* Known values */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_cipher_suite_from_iana(null_iana, sizeof(null_iana), &cipher_suite), S2N_ERR_CIPHER_NOT_SUPPORTED);
        ...
        cipher_suite = NULL;
        EXPECT_OK(s2n_cipher_suite_from_iana(
            s2n_tls13_aes_256_gcm_sha384.iana_value, sizeof(s2n_tls13_aes_256_gcm_sha384.iana_value), &cipher_suite));
    ...
    /* Conversion is correct for all supported cipher suites */
    {
        ...
        for (size_t i = 0; i < cipher_preferences_test_all.count; i++) {
            ...
            EXPECT_OK(s2n_cipher_suite_from_iana(
                expected_cipher_suite->iana_value, sizeof(expected_cipher_suite->iana_value), &actual_cipher_suite));
        ...
        for (size_t i0 = 0; i0 <= UINT8_MAX; i0++) {
            ...
            for (size_t i1 = 0; i1 <= UINT8_MAX; i1++) {
                ...
                bool is_supported = supported_i < cipher_preferences_test_all.count
                    && memcmp(iana_value, cipher_preferences_test_all.suites[supported_i]->iana_value,
