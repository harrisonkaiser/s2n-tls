...
int main(int argc, char **argv)
{
    ...
    /* test invalid date fails */
    {
        ...
        EXPECT_ERROR_WITH_ERRNO(
            s2n_asn1_time_to_nano_since_epoch_ticks(time_str, strlen(time_str), &timestamp), S2N_ERR_INVALID_ARGUMENT);
    }
    ...
    /* Test UTC time before Epoch fails */
    {
        ...
        uint64_t timestamp = 0;
        EXPECT_ERROR_WITH_ERRNO(
            s2n_asn1_time_to_nano_since_epoch_ticks(time_str, strlen(time_str), &timestamp), S2N_ERR_SAFETY);
    }
