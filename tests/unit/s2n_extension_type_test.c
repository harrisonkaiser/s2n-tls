...
int main()
{
    ...
    /* Test s2n_extension_supported_iana_value_to_id */
    {
         ...
         * 15 == heartbeat, which s2n will probably never support :) */
        EXPECT_FAILURE_WITH_ERRNO(s2n_extension_supported_iana_value_to_id(15, &id), S2N_ERR_UNRECOGNIZED_EXTENSION);
    }
    ...
    /* Test s2n_extension_recv */
    {
         ...
         **/
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_extension_recv(&response_extension_type, &conn, &stuffer), S2N_ERR_UNSUPPORTED_EXTENSION);
        ...
        /* "recv" errors */
        {
            ...
            EXPECT_FAILURE_WITH_ERRNO(
                s2n_extension_recv(&extension_type_with_failure, &conn, &stuffer), S2N_ERR_UNIMPLEMENTED);
            /* cppcheck-suppress sizeofDivisionMemfunc */
            EXPECT_BITFIELD_CLEAR(conn.extension_requests_received);
        }
    }
...
/* "send" writes more data than will fit in the extension size */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_extension_send(&extension_type_with_too_much_data, &conn, &stuffer), S2N_ERR_SIZE_MISMATCH);
...
/* Test error if_missing */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_extension_is_missing(&extension_type_with_error_if_missing, &conn), S2N_ERR_MISSING_EXTENSION);
...
/* Test minimum_version field */
{
    ...
    /* Functional test: minimum-TLS1.3 extensions only used for TLS1.3 */
    {
        ...
        if (s2n_is_tls13_fully_supported()) {
            ...
            EXPECT_OK(
                s2n_negotiate_test_server_and_client_until_message(server_conn, client_conn, ENCRYPTED_EXTENSIONS));
