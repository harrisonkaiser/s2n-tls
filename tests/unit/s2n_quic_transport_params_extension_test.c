...
int main(int argc, char **argv)
{
    ...
    /* Test if_missing */
    {
         ...
         **/
        EXPECT_SUCCESS(s2n_config_enable_quic(config));
        EXPECT_FAILURE_WITH_ALERT(s2n_quic_transport_parameters_extension.if_missing(conn), S2N_ERR_MISSING_EXTENSION,
            S2N_TLS_ALERT_MISSING_EXTENSION);
...
/* Test recv */
{
    /* Safety checks */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_quic_transport_parameters_extension.recv(conn, &extension), S2N_ERR_UNSUPPORTED_EXTENSION);
