...
const struct s2n_tls13_cert_verify_test test_cases[] = {
    { .cert_file = S2N_ECDSA_P384_PKCS1_CERT_CHAIN,
        .key_file = S2N_ECDSA_P384_PKCS1_KEY,
...
int run_tests(const struct s2n_tls13_cert_verify_test *test_case, s2n_mode verifier_mode)
{
    ...
    /* Successfully send and receive certificate verify */
    {
        ...
        if (verifying_conn->mode == S2N_CLIENT) {
            EXPECT_SUCCESS(s2n_asn1der_to_public_key_and_type(
                &verifying_conn->handshake_params.server_public_key, &pkey_type, &b));
            EXPECT_SUCCESS(s2n_pkey_match(&verifying_conn->handshake_params.server_public_key,
                sending_conn->handshake_params.our_chain_and_key->private_key));
        } else {
            EXPECT_SUCCESS(s2n_asn1der_to_public_key_and_type(
                &verifying_conn->handshake_params.client_public_key, &pkey_type, &b));
            EXPECT_SUCCESS(s2n_pkey_match(&verifying_conn->handshake_params.client_public_key,
                sending_conn->handshake_params.our_chain_and_key->private_key));
        }
    ...
    /* Verifying connection errors with incorrect signed content */
    {
        ...
        if (verifying_conn->mode == S2N_CLIENT) {
            EXPECT_SUCCESS(s2n_asn1der_to_public_key_and_type(
                &verifying_conn->handshake_params.server_public_key, &pkey_type, &b));
            EXPECT_SUCCESS(s2n_pkey_match(&verifying_conn->handshake_params.server_public_key,
                verifying_conn->handshake_params.our_chain_and_key->private_key));
        } else {
            EXPECT_SUCCESS(s2n_asn1der_to_public_key_and_type(
                &verifying_conn->handshake_params.client_public_key, &pkey_type, &b));
            EXPECT_SUCCESS(s2n_pkey_match(&verifying_conn->handshake_params.client_public_key,
    ...
    /* Verifying connection errors with wrong hash/signature algorithms */
    {
        ...
        EXPECT_FAILURE(s2n_tls13_cert_read_and_verify_signature(
            verifying_conn, &verifying_conn->handshake_params.conn_sig_scheme));
