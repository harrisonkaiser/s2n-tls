...
int main(int argc, char **argv)
{
     ...
     *       and matches the non-MIDDLEBOX_COMPAT form EXCEPT for CCS messages */
    {
        ...
        for (size_t i = 0; i < valid_tls13_handshakes_size; i++) {
            ...
            for (size_t j = 0, j_mc = 0; j < S2N_MAX_HANDSHAKE_LENGTH && j_mc < S2N_MAX_HANDSHAKE_LENGTH; j++, j_mc++) {
                ...
                while (
                    messages_mc[j_mc] == SERVER_CHANGE_CIPHER_SPEC || messages_mc[j_mc] == CLIENT_CHANGE_CIPHER_SPEC) {
                    j_mc++;
                }
     ...
     * Any handshake could start with early data, even if that early data is later rejected. */
    {
        ...
        for (size_t i = 0; i < valid_tls13_handshakes_size; i++) {
            ...
            for (size_t j = 0, j_test = 0; j < S2N_MAX_HANDSHAKE_LENGTH && j_test < S2N_MAX_HANDSHAKE_LENGTH;
                 j++, j_test++) {
     ...
     */
{
    ...
    for (size_t i = 0; i < valid_tls13_handshakes_size; i++) {
        ...
        for (size_t j = 1; j < S2N_MAX_HANDSHAKE_LENGTH; j++) {
            ...
            if (tls13_state_machine[messages[j]].writer != 'C' || tls13_state_machine[messages[j - 1]].writer != 'S') {
                continue;
            }
    ...
    for (size_t i = 0; i < valid_tls13_handshakes_size; i++) {
        ...
        for (size_t j = 1; j < S2N_MAX_HANDSHAKE_LENGTH; j++) {
            ...
            if (tls13_state_machine[messages[j]].writer != 'S' || tls13_state_machine[messages[j - 1]].writer != 'C') {
                continue;
            }
...
/* Test: TLS 1.3 handshake types are all properly printed */
{
    ...
    const char *all_flags_handshake_type_name =
        "NEGOTIATED|FULL_HANDSHAKE|CLIENT_AUTH|NO_CLIENT_CERT"
        "|MIDDLEBOX_COMPAT|WITH_EARLY_DATA|EARLY_CLIENT_CCS";
    conn->handshake.handshake_type = NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | NO_CLIENT_CERT | MIDDLEBOX_COMPAT
        | WITH_EARLY_DATA | EARLY_CLIENT_CCS;
...
/* Test: TLS 1.3 message types are all properly printed */
{
    ...
    const char *expected[] = { "CLIENT_HELLO", "SERVER_HELLO", "SERVER_CHANGE_CIPHER_SPEC", "ENCRYPTED_EXTENSIONS",
        "SERVER_CERT", "SERVER_CERT_VERIFY", "SERVER_FINISHED", "CLIENT_CHANGE_CIPHER_SPEC", "CLIENT_FINISHED",
        "APPLICATION_DATA" };
...
/* Test: TLS 1.3 message types are all properly printed for client auth */
{
    ...
    const char *expected[] = { "CLIENT_HELLO", "SERVER_HELLO", "ENCRYPTED_EXTENSIONS", "SERVER_CERT_REQ", "SERVER_CERT",
        "SERVER_CERT_VERIFY", "SERVER_FINISHED", "CLIENT_CERT", "CLIENT_CERT_VERIFY", "CLIENT_FINISHED",
        "APPLICATION_DATA" };
