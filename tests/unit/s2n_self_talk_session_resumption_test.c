...
#define ARE_FULL_HANDSHAKES(client, server) (IS_FULL_HANDSHAKE(client) && IS_FULL_HANDSHAKE(server))
...
static S2N_RESULT s2n_wipe_connections(
    struct s2n_connection *client_conn, struct s2n_connection *server_conn, struct s2n_test_io_pair *io_pair)
{
...
static int s2n_cache_retrieve_cb(
    struct s2n_connection *conn, void *ctx, const void *key, uint64_t key_size, void *value, uint64_t *value_size)
{
    return S2N_SUCCESS;
}

static int s2n_cache_store_cb(struct s2n_connection *conn, void *ctx, uint64_t ttl_in_seconds, const void *key,
...
static S2N_RESULT s2n_test_issue_new_session_ticket(struct s2n_connection *server_conn,
    struct s2n_connection *client_conn, const struct s2n_early_data_test_case *early_data_case)
{
    ...
    const struct s2n_early_data_test_case *early_data_case)
{
    ...
    RESULT_GUARD(s2n_negotiate_test_server_and_client_with_early_data(
        server_conn, client_conn, &early_data_send, &early_data_recv));
...
int main(int argc, char **argv)
{
    ...
    for (size_t ticket_supported = 0; ticket_supported < 2; ticket_supported++) {
        ...
        for (size_t client_supported = 0; client_supported < 2; client_supported++) {
            ...
            for (size_t server_supported = 0; server_supported < 2; server_supported++) {
                ...
                early_data_test_cases[test_case_i].expect_success =
                    client_supported && server_supported && ticket_supported;
            }
        }
        test_case_i++;
    }
    ...
    const struct s2n_early_data_test_case no_early_data = {
        .client_supported = false, .server_supported = false, .expect_success = false
    };
    ...
    EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(
        &tls13_chain_and_key, S2N_DEFAULT_ECDSA_TEST_CERT_CHAIN, S2N_DEFAULT_ECDSA_TEST_PRIVATE_KEY));
     ...
     */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_negotiate_test_server_and_client(server_conn, client_conn), S2N_ERR_PROTOCOL_DOWNGRADE_DETECTED);
     ...
     */
    {
        ...
        for (size_t i = 0; i < 10; i++) {
            ...
            if (do_tls13) {
                ...
                EXPECT_SUCCESS(s2n_connection_set_config(client_conn, tls13_client_config));
                EXPECT_SUCCESS(s2n_connection_set_session(
                    client_conn, tls13_ticket.blob.data, s2n_stuffer_data_available(&tls13_ticket)));
            } else {
                expected_version = S2N_TLS12;
                EXPECT_SUCCESS(s2n_connection_set_config(client_conn, tls12_client_config));
                EXPECT_SUCCESS(s2n_connection_set_session(
                    client_conn, tls12_ticket.blob.data, s2n_stuffer_data_available(&tls12_ticket)));
            }
            ...
            if (do_tls13) {
                ...
                EXPECT_OK(s2n_test_issue_new_session_ticket(server_conn, client_conn, &no_early_data));
                EXPECT_SUCCESS(
                    s2n_stuffer_copy(&cb_session_data, &tls13_ticket, s2n_stuffer_data_available(&cb_session_data)));
            } else {
     ...
     */
    {
        ...
        /* TLS1.2 */
        {
            ...
            /* Initial handshake */
            {
                ...
                EXPECT_TRUE(IS_ISSUING_NEW_SESSION_TICKET(server_conn));
                EXPECT_SUCCESS(
                    s2n_stuffer_copy(&cb_session_data, &ticket, s2n_stuffer_data_available(&cb_session_data)));
            ...
            /* Resumption handshake */
            {
                EXPECT_SUCCESS(s2n_connection_set_config(server_conn, server_config));
                EXPECT_SUCCESS(
                    s2n_connection_set_session(client_conn, ticket.blob.data, s2n_stuffer_data_available(&ticket)));
