    ...
    if (!s2n_pq_is_enabled()) {
        ...
        if (test_security_policy.kem_preferences->tls13_kem_group_count >= 2) {
            ...
            EXPECT_SUCCESS(s2n_stuffer_alloc_ro_from_hex_string(&hrr,
                "0303CF21AD74E59A6111BE1D8C021E65B891C2A211167ABB8C5E079E09E2C8A8339C00130200000C002B00020304003300022F"
                "39"));
     ...
     **/
{
    ...
    EXPECT_MEMCPY_SUCCESS(
        conn->handshake_params.server_random, not_hello_retry_request_random, S2N_TLS_RANDOM_DATA_LEN);
...
if (s2n_is_tls13_fully_supported()) {
    ...
    for (size_t tls13_tickets = 0; tls13_tickets < 2; tls13_tickets++) {
        ...
        for (size_t i = 0; i < extensions->count; i++) {
            ...
            if (tls13_tickets && iana == TLS_EXTENSION_SESSION_TICKET) {
                ...
                && (iana == TLS_EXTENSION_PRE_SHARED_KEY || iana == TLS_EXTENSION_PSK_KEY_EXCHANGE_MODES
            ...
            EXPECT_SUCCESS(s2n_client_hello_has_extension(&server_conn->client_hello, iana, &extension_exists));
         ...
         */
    {
        ...
        DEFER_CLEANUP(struct s2n_config *config_with_cb = s2n_config_new(), s2n_config_ptr_free);
        ...
        EXPECT_SUCCESS(
            s2n_config_set_client_hello_cb(config_with_cb, s2n_client_hello_cb_with_get_server_name, server_name));
     ...
     **/
{
         ...
         **/
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_negotiate_test_server_and_client(server_conn, client_conn), S2N_ERR_CIPHER_NOT_SUPPORTED);
    }
     ...
     **/
{
    ...
    /* Client receives HelloRetryRequest */
    EXPECT_FAILURE_WITH_ERRNO(s2n_server_hello_recv(client_conn), S2N_ERR_MISSING_EXTENSION);
}
     ...
     **/
{
    ...
    struct s2n_cipher_suite *test_cipher_suites[] = { &s2n_tls13_aes_128_gcm_sha256, &s2n_tls13_aes_256_gcm_sha384 };
     ...
     **/
{
    ...
    EXPECT_ERROR_WITH_ERRNO(
        s2n_negotiate_until_message(client_conn, &blocked, ENCRYPTED_EXTENSIONS), S2N_ERR_BAD_MESSAGE);
}
