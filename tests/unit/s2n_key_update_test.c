...
EXPECT_FAILURE_WITH_ALERT(s2n_key_update_recv(conn, &input), S2N_ERR_BAD_MESSAGE, S2N_TLS_ALERT_UNEXPECTED_MESSAGE);
...
/* Server receives valid key update request */
{
    ...
    POSIX_CHECKED_MEMCPY(
        server_conn->secrets.tls13.client_app_secret, application_secret.data, application_secret.size);
...
/* Client receives valid key update request */
{
    ...
    POSIX_CHECKED_MEMCPY(
        client_conn->secrets.tls13.server_app_secret, application_secret.data, application_secret.size);
...
/* Key update is triggered by encryption limits */
{
    ...
    client_conn->secure->cipher_suite = cipher_suite_with_limit;
    POSIX_CHECKED_MEMCPY(
        client_conn->secrets.tls13.client_app_secret, application_secret.data, application_secret.size);
    ...
    EXPECT_BYTEARRAY_EQUAL(
        client_conn->secure->client_sequence_number, zeroed_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
    EXPECT_TRUE(s2n_stuffer_data_available(&stuffer) > 0);
...
/* Key update is not triggered */
{
    ...
    EXPECT_EQUAL(client_conn->key_update_pending, false);
    EXPECT_BYTEARRAY_EQUAL(
        client_conn->secure->client_sequence_number, expected_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
    EXPECT_TRUE(s2n_stuffer_data_available(&stuffer) == 0);
...
/* Key update eventually occurs when record limit reached */
{
    ...
    EXPECT_SUCCESS(
        s2n_blob_init(&sequence_number, client_conn->secure->client_sequence_number, S2N_TLS_SEQUENCE_NUM_LEN));
