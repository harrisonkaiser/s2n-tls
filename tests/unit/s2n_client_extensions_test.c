...
static int negotiate_kem(const uint8_t client_extensions[], const size_t client_extensions_len,
    const uint8_t client_hello_message[], const size_t client_hello_len, const char cipher_pref_version[],
    const int expected_kem_id, struct s2n_test_io_pair *io_pair)
{
    char *cert_chain;
    char *private_key;
...
int main(int argc, char **argv)
{
    ...
    /* Client sends multiple server names. */
    {
        ...
        EXPECT_SUCCESS(s2n_read_test_pem_and_len(
            S2N_DEFAULT_TEST_CERT_CHAIN, (uint8_t *) cert_chain, &cert_chain_len, S2N_MAX_TEST_PEM_SIZE));
        EXPECT_SUCCESS(s2n_read_test_pem_and_len(
            S2N_DEFAULT_TEST_PRIVATE_KEY, (uint8_t *) private_key, &private_key_len, S2N_MAX_TEST_PEM_SIZE));
        EXPECT_NOT_NULL(chain_and_key = s2n_cert_chain_and_key_new());
        EXPECT_SUCCESS(s2n_cert_chain_and_key_load_pem_bytes(
            chain_and_key, (uint8_t *) cert_chain, cert_chain_len, (uint8_t *) private_key, private_key_len));
        EXPECT_SUCCESS(s2n_config_add_cert_chain_and_key_to_store(server_config, chain_and_key));
        EXPECT_SUCCESS(s2n_connection_set_config(server_conn, server_config));
        ...
        EXPECT_EQUAL(
            write(io_pair.client, client_hello_message, sizeof(client_hello_message)), sizeof(client_hello_message));
    ...
    /* Client doesn't use the OCSP extension. */
    {
        ...
        EXPECT_SUCCESS(s2n_config_set_extension_data(
            server_config, S2N_EXTENSION_OCSP_STAPLING, server_ocsp_status, sizeof(server_ocsp_status)));
        EXPECT_SUCCESS(s2n_connection_set_config(server_conn, server_config));
    ...
    /* Client does not request SCT, but server is configured to serve them. */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_cert_chain_and_key(server_config, cert_chain, private_key));
        EXPECT_SUCCESS(s2n_config_set_extension_data(
            server_config, S2N_EXTENSION_CERTIFICATE_TRANSPARENCY, sct_list, sizeof(sct_list)));
        EXPECT_SUCCESS(s2n_connection_set_config(server_conn, server_config));
    ...
    /* All PQ KEM byte values are from https://tools.ietf.org/html/draft-campagna-tls-bike-sike-hybrid */
    {
        ...
        EXPECT_SUCCESS(negotiate_kem(client_extensions, client_extensions_len, client_hello_message, client_hello_len,
            "PQ-TLS-1-0-2021-05-24", expected_kem_id, &io_pair));
    }

    {
        ...
        size_t client_hello_len = sizeof(client_hello_message);
        EXPECT_SUCCESS(negotiate_kem(client_extensions, client_extensions_len, client_hello_message, client_hello_len,
            "KMS-PQ-TLS-1-0-2019-06", expected_kem_id, &io_pair));
    }
