...
#define IS_CLIENT_AUTH(client, server) (IS_CLIENT_AUTH_HANDSHAKE(client) && IS_CLIENT_AUTH_HANDSHAKE(server))
...
static s2n_result validate_chosen_psk(
    struct s2n_connection *server_conn, uint8_t *psk_identity_data, size_t psk_identity_size, size_t chosen_index)
{
    RESULT_ENSURE_REF(server_conn);
    ...
    RESULT_ENSURE_EQ(
        memcmp(server_conn->psk_params.chosen_psk->identity.data, psk_identity_data, psk_identity_size), 0);
...
int main(int argc, char **argv)
{
    ...
    /* Basic PSK without callback function, without certificates and client auth not set */
    {
        ...
        /* Validate chosen PSK */
        EXPECT_OK(validate_chosen_psk(
            server_conn, test_shared_identity, sizeof(test_shared_identity), TEST_SHARED_PSK_WIRE_INDEX_1));
    ...
    /* PSK with callback function */
    {
        ...
        EXPECT_SUCCESS(
            s2n_config_set_psk_selection_callback(server_conn->config, s2n_test_select_psk_identity_callback, NULL));
        ...
        /* Validate chosen PSK */
        EXPECT_OK(validate_chosen_psk(
            server_conn, test_shared_identity_2, sizeof(test_shared_identity_2), TEST_SHARED_PSK_WIRE_INDEX_2));
    ...
    /* Basic PSK with Client Hello async callback set */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_negotiate_test_server_and_client(server_conn, client_conn), S2N_ERR_ASYNC_BLOCKED);
