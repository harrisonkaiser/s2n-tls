...
/* renegotiation_info is non-empty after wipe */
{
    ...
    ssize_t renegotiation_info_len =
        s2n_client_hello_get_extension_length(&server_conn->client_hello, S2N_EXTENSION_RENEGOTIATION_INFO);
...
/* Wipe mid-write not allowed */
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_send(client_conn, large_app_data, sizeof(large_app_data), &blocked), S2N_ERR_IO_BLOCKED);
