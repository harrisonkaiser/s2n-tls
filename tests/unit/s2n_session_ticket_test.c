...
int main(int argc, char **argv)
{
    ...
    uint8_t serialized_session_state[S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TLS12_TICKET_SIZE_IN_BYTES
        + S2N_TLS12_STATE_SIZE_IN_BYTES] = { 0 };
    ...
    uint8_t ticket_key1[32] = { 0x07, 0x77, 0x09, 0x36, 0x2c, 0x2e, 0x32, 0xdf, 0x0d, 0xdc, 0x3f, 0x0d, 0xc4, 0x7b,
        0xba, 0x63, 0x90, 0xb6, 0xc7, 0x3b, 0xb5, 0x0f, 0x9c, 0x31, 0x22, 0xec, 0x84, 0x4a, 0xd7, 0xc2, 0xb3, 0xe5 };
    uint8_t ticket_key2[32] = { 0x06, 0xa6, 0xb8, 0x8c, 0x58, 0x53, 0x36, 0x1a, 0x06, 0x10, 0x4c, 0x9c, 0xeb, 0x35,
        0xb4, 0x5c, 0xef, 0x76, 0x00, 0x14, 0x90, 0x46, 0x71, 0x01, 0x4a, 0x19, 0x3f, 0x40, 0xc1, 0x5f, 0xc2, 0x44 };
    uint8_t ticket_key3[32] = { 0x19, 0xef, 0x24, 0xa3, 0x2c, 0x71, 0x7b, 0x16, 0x7f, 0x33, 0xa9, 0x1d, 0x6f, 0x64,
        0x8b, 0xdf, 0x96, 0x59, 0x67, 0x76, 0xaf, 0xdb, 0x63, 0x77, 0xac, 0x43, 0x4c, 0x1c, 0x29, 0x3c, 0xcb, 0x04 };
    ...
    /* Test ticket name handling */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_config_add_ticket_crypto_key(config, too_large_name, 0, ticket_key1, s2n_array_len(ticket_key1), 0),
            S2N_ERR_INVALID_TICKET_KEY_NAME_OR_NAME_LENGTH);
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_config_add_ticket_crypto_key(config, too_large_name, 17, ticket_key1, s2n_array_len(ticket_key1), 0),
            S2N_ERR_INVALID_TICKET_KEY_NAME_OR_NAME_LENGTH);

        /* Add a ticket with a single-byte name */
        struct small_name_ticket small = { .name = { 0xAA }, .key = { 0xBB, 0xBB, 0xBB, 0xBB } };
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
            config, small.name, s2n_array_len(small.name), small.key, s2n_array_len(small.key), 0));

        /* Ensure a ticket with the same name is not added */
        struct small_name_ticket small2 = { .name = { 0xAA }, .key = { 0xCC, 0xCC, 0xCC, 0xCC } };
        EXPECT_FAILURE_WITH_ERRNO(s2n_config_add_ticket_crypto_key(config, small2.name, s2n_array_len(small2.name),
                                      small2.key, s2n_array_len(small2.key), 0),
            S2N_ERR_INVALID_TICKET_KEY_NAME_OR_NAME_LENGTH);

        /* Ensure a ticket with a zero-padded name is not added */
        uint8_t padded_name[16] = { 0xAA, 0 };
        EXPECT_FAILURE_WITH_ERRNO(s2n_config_add_ticket_crypto_key(config, padded_name, s2n_array_len(padded_name),
    ...
    /* Client sends empty ST extension. Server issues NST. */
    {
        ...
        EXPECT_SUCCESS(
            s2n_config_set_session_state_lifetime(server_config, S2N_SESSION_STATE_CONFIGURABLE_LIFETIME_IN_SECS));

        /* Add one ST key */
        POSIX_GUARD(server_config->wall_clock(server_config->sys_clock_ctx, &now));
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1,
            s2n_array_len(ticket_key_name1), ticket_key1, s2n_array_len(ticket_key1), now / ONE_SEC_IN_NANOS));
        ...
        EXPECT_EQUAL(s2n_connection_get_session_ticket_lifetime_hint(client_conn),
    ...
    /* Client sends non-empty ST extension. Server does an abbreviated handshake without issuing NST. */
    {
        ...
        EXPECT_SUCCESS(
            s2n_connection_set_session(client_conn, serialized_session_state, serialized_session_state_length));
        ...
        EXPECT_MEMCPY_SUCCESS(old_session_ticket, serialized_session_state,
        ...
        EXPECT_BYTEARRAY_EQUAL(old_session_ticket, serialized_session_state,
            S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TLS12_TICKET_SIZE_IN_BYTES);
     ...
     */
    {
        ...
        /* Add a second ST key */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2,
            s2n_array_len(ticket_key_name2), ticket_key2, s2n_array_len(ticket_key2), 0));
        ...
        s2n_connection_get_session(client_conn, serialized_session_state, serialized_session_state_length);
        EXPECT_TRUE(memcmp(old_session_ticket, serialized_session_state,
            S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TLS12_TICKET_SIZE_IN_BYTES));
        ...
        EXPECT_BYTEARRAY_EQUAL(serialized_session_state + S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES, ticket_key_name2,
     ...
     */
    {
        ...
        s2n_connection_get_session(client_conn, serialized_session_state, serialized_session_state_length);
        EXPECT_FALSE(memcmp(old_session_ticket, serialized_session_state,
            S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TLS12_TICKET_SIZE_IN_BYTES));
    ...
    /* Client sends non-empty ST extension. Server does a full handshake and issues a NST because the ticket has non-standard size. */
    {
        ...
        EXPECT_MEMCPY_SUCCESS(
            tampered_session_state + 3, serialized_session_state + 4, sizeof(tampered_session_state) - 4);

        /* Set client tampered ST and session state */
        EXPECT_SUCCESS(
            s2n_connection_set_session(client_conn, tampered_session_state, serialized_session_state_length - 1));
    ...
    /* Client sends non-empty ST extension, but server cannot or does not want to honor the ticket. */
    {
        ...
        EXPECT_EQUAL(s2n_connection_get_session(client_conn, serialized_session_state, serialized_session_state_length),
            1 + 1 + client_conn->session_id_len + S2N_TLS12_STATE_SIZE_IN_BYTES);
        EXPECT_EQUAL(memcmp(serialized_session_state, &s2n_state_with_session_id, 1), 0);
        EXPECT_NOT_EQUAL(memcmp(serialized_session_state + S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES, ticket_key_name2,
                             s2n_array_len(ticket_key_name2)),
            0);

        /* Verify the lifetime hint from the server */
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_connection_get_session_ticket_lifetime_hint(client_conn), S2N_ERR_SESSION_TICKET_NOT_SUPPORTED);
    ...
    /* User tries adding a duplicate key to the server */
    {
        ...
        EXPECT_EQUAL(-1,
            s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2, s2n_array_len(ticket_key_name2),
                ticket_key1, s2n_array_len(ticket_key1), 0));

        /* Try adding a different key, but with the same name */
        EXPECT_EQUAL(-1,
            s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1, s2n_array_len(ticket_key_name1),
        ...
        EXPECT_EQUAL(-1,
            s2n_config_add_ticket_crypto_key(
                server_config, ticket_key_name2, s2n_array_len(ticket_key_name2), ticket_key2, 0, 0));
    ...
    /* Testing expired keys are removed from the server config while adding new keys. */
    {
        ...
        /* Add a third ST key */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name3,
            s2n_array_len(ticket_key_name3), ticket_key3, s2n_array_len(ticket_key3), 0));

        /* Try adding the expired keys */
        EXPECT_EQUAL(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2, s2n_array_len(ticket_key_name2),
                         ticket_key2, s2n_array_len(ticket_key2), 0),
            -1);
        EXPECT_EQUAL(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1, s2n_array_len(ticket_key_name1),
                         ticket_key1, s2n_array_len(ticket_key1), 0),
            -1);
    ...
    /* Add keys out of order and pre-emptively add a key */
    {
        ...
        POSIX_GUARD(server_config->wall_clock(server_config->sys_clock_ctx, &now));
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1,
            s2n_array_len(ticket_key_name1), ticket_key1, s2n_array_len(ticket_key1), (now / ONE_SEC_IN_NANOS) + 3600));

        /* Add a key. After 1 hour it will reach it's peak */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2,
            s2n_array_len(ticket_key_name2), ticket_key2, s2n_array_len(ticket_key2), now / ONE_SEC_IN_NANOS));

        /* Add a key pre-emptively. It can be used only after 10 hours */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name3,
            s2n_array_len(ticket_key_name3), ticket_key3, s2n_array_len(ticket_key3), now / ONE_SEC_IN_NANOS + 36000));
    ...
    /* s2n_decrypt_session_ticket fails to decrypt when presented with a valid ticket_key, valid iv and invalid encrypted blob */
    {
        ...
        POSIX_GUARD(s2n_stuffer_write_bytes(
            &server_conn->client_ticket_to_decrypt, ticket_key_name1, s2n_array_len(ticket_key_name1)));
        ...
        POSIX_GUARD(
            s2n_stuffer_write_bytes(&server_conn->client_ticket_to_decrypt, invalid_en_data, sizeof(invalid_en_data)));

        server_conn->session_ticket_status = S2N_DECRYPT_TICKET;
        EXPECT_FAILURE_WITH_ERRNO(
            s2n_decrypt_session_ticket(server_conn, &server_conn->client_ticket_to_decrypt), S2N_ERR_DECRYPT);
    ...
    /* s2n_decrypt_session_ticket fails with a key not found error when presented with an invalid ticket_key, valid iv and invalid encrypted blob */
    {
        ...
        POSIX_GUARD(s2n_stuffer_write_bytes(
            &server_conn->client_ticket_to_decrypt, ticket_key_name2, s2n_array_len(ticket_key_name2)));
        ...
        server_conn->session_ticket_status = S2N_DECRYPT_TICKET;
        EXPECT_FAILURE_WITH_ERRNO(s2n_decrypt_session_ticket(server_conn, &server_conn->client_ticket_to_decrypt),
     ...
     */
{
    ...
    EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1, s2n_array_len(ticket_key_name1),
        ticket_key1, s2n_array_len(ticket_key1), now / ONE_SEC_IN_NANOS));
...
if (s2n_is_tls13_fully_supported()) {
    ...
    EXPECT_SUCCESS(s2n_config_set_session_tickets_onoff(config, 1));
    EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
        config, ticket_key_name1, s2n_array_len(ticket_key_name1), ticket_key1, s2n_array_len(ticket_key1), 0));
...
if (s2n_is_tls13_fully_supported()) {
    ...
    EXPECT_SUCCESS(
        s2n_extension_supported_iana_value_to_id(TLS_EXTENSION_SESSION_TICKET, &client_session_ticket_ext_id));
