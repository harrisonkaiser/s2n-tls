...
static void benchmark_single_suite_client(benchmark::State &state)
{
     ...
     * created */
    struct s2n_cipher_suite *cipher_suites_benchmark[] = { all_suites[state.range(0)] };
...
int start_negotiate_benchmark_client(int argc, char **argv)
{
    ...
    for (long int current_suite = 0; current_suite < num_suites; current_suite++) {
        ...
        benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_single_suite_client)
            ->Repetitions(repetitions)
            ->ReportAggregatesOnly()
            ->Iterations(iterations)
            ->Args({ current_suite, warmup_iters, sockfd });
    }
