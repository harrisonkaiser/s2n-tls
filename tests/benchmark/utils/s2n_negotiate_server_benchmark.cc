...
int start_negotiate_benchmark_server(int argc, char **argv)
{
    ...
    argument_parse(argc, argv, use_corked_io, insecure, bench_format, file_prefix, warmup_iters, iterations,
        repetitions, gb_options, data_sizes);
    ...
    GUARD_EXIT(s2n_config_add_cert_chain_and_key_to_store(config, chain_and_key_rsa), "Error adding RSA chain and key");
    ...
    GUARD_EXIT(
        s2n_config_add_cert_chain_and_key_to_store(config, chain_and_key_ecdsa), "Error adding ECDSA chain and key");
    ...
    for (long int suite_num = 0; suite_num < num_suites; ++suite_num) {
        ...
        benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_single_suite_server)
            ->Repetitions(repetitions)
            ->Iterations(iterations)
            ->Args({ suite_num, warmup_iters, connectionfd });
    }
