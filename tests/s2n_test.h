...
#define EXPECT_TRUE_WITHOUT_COUNT(condition)      \
    do {                                          \
        if (!(condition)) {                       \
            FAIL_MSG(#condition " is not true "); \
        }                                         \
    } while (0)
...
#define FAIL_MSG_PRINT(msg)                                                                                           \
    do {                                                                                                              \
        ...
        if (isatty(fileno(stderr))) {                                                                                 \
            errno = real_errno;                                                                                       \
            fprintf(stderr,                                                                                           \
                "\033[31;1mFAILED test %d\033[0m\n%s (%s:%d)\nError Message: '%s'\n Debug String: '%s'\n System "     \
                "Error: %s (%d)\n",                                                                                   \
                test_count, (msg), __FILE__, __LINE__, s2n_strerror(s2n_errno, "EN"), s2n_debug_str, strerror(errno), \
                errno);                                                                                               \
        } else {                                                                                                      \
            errno = real_errno;                                                                                       \
            fprintf(stderr,                                                                                           \
...
#define S2N_FUZZ_ENSURE_MIN_LEN(len, min) \
    do {                                  \
        if ((len) < (min))                \
            return S2N_SUCCESS;           \
    } while (0)
