...
static __thread struct s2n_rand_state s2n_per_thread_rand_state = { .cached_fork_generation_number = 0,
    .public_drbg = { 0 },
    .private_drbg = { 0 },
...
int s2n_rand_set_callbacks(s2n_rand_init_callback rand_init_callback, s2n_rand_cleanup_callback rand_cleanup_callback,
...
static S2N_RESULT s2n_get_random_data(struct s2n_blob *out_blob, struct s2n_drbg *drbg_state)
{
    RESULT_GUARD(s2n_ensure_initialized_drbgs());
