...
S2N_RESULT s2n_array_insert(struct s2n_array *array, uint32_t idx, void **element)
{
    ...
    if (idx < array->len) {
        ...
        memmove(array->mem.data + array->element_size * (idx + 1), array->mem.data + array->element_size * idx, size);
    }
...
S2N_RESULT s2n_array_remove(struct s2n_array *array, uint32_t idx)
{
    ...
    if (idx != array->len - 1) {
        ...
        memmove(array->mem.data + array->element_size * idx, array->mem.data + array->element_size * (idx + 1), size);
    }
    array->len--;

    /* After shifting, zero the last element */
    RESULT_CHECKED_MEMSET(array->mem.data + array->element_size * array->len, 0, array->element_size);
