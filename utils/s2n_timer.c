...
{
    RESULT_ENSURE(config->monotonic_clock(config->monotonic_clock_ctx, &timer->time) >= S2N_SUCCESS, S2N_ERR_CANCELLED);
...
S2N_RESULT s2n_timer_elapsed(struct s2n_config *config, struct s2n_timer *timer, uint64_t *nanoseconds)
{
    ...
    RESULT_ENSURE(
        config->monotonic_clock(config->monotonic_clock_ctx, &current_time) >= S2N_SUCCESS, S2N_ERR_CANCELLED);
